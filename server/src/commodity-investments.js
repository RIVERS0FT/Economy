...
export function commodityInvestmentValuation(world, player, now) {
  ...
  return {
    ...
    realizedProfit: account.realizedProfit, feesPaid: account.feesPaid, closeGross: account.closeGross,
    positions, recentTransactions: structuredClone(account.recentTransactions),
  };
}

/** Internal debt recovery. Close the smallest whole-unit quantities that cover the remaining claim. */
export function collectCommodityInvestments(world, player, targetCredits, now) {
  cashTimestamp(now);
  let target = cashMicros(targetCredits, '追偿目标');
  if (target === 0n) return { transactions: [] };
  const draft = draftFor(player);
  const candidates = Object.values(draft.account.positions).sort((a, b) => a.contractId.localeCompare(b.contractId));
  const transactions = [];
  for (const position of candidates) {
    if (target <= 0n) break;
    const at = Math.min(now, position.expiresAt);
    const quote = commodityInvestmentQuote(world, position.productId, at);
    const payoutFor = (quantity) => {
      const gross = cashMicros(cashProduct(quote.price, quantity));
      const fee = cashFee(cashMicros(draft.account.closeGross) + gross, COMMODITY_INVESTMENT_FEE_BPS)
        - cashMicros(draft.account.feesPaid);
      return gross - fee;
    };
    let left = 1; let right = position.quantity;
    while (left < right) {
      const middle = left + Math.floor((right - left) / 2);
      if (payoutFor(middle) >= target) right = middle;
      else left = middle + 1;
    }
    const row = closePosition(draft, position, left, quote, at, now, 'collection');
    transactions.push(row);
    target -= cashMicros(row.cashChange);
  }
  if (transactions.length) commit(player, draft);
