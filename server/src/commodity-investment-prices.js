...
/** Validate the complete daily batch before publishing one archive, copying each date at most once. */
export function archiveOfficialCashMarkets(world, markets) {
  ...
  for (const market of markets) {
    const provinceId = requireCashProvince(market.provinceId);
    const productId = requireCashProduct(market.productId);
    const dateKey = market.priceDateKey;
    if (typeof dateKey !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) {
      throw cashEconomyError('CASH_PRICE_INVALID', '正式价格日期无效');
    }
    const price = cashMoney(priceMicros(market.officialPrice));
    const key = `${provinceId}:${productId}`;
    const previous = changedDays.get(dateKey) ?? archive.days[dateKey]?.prices ?? {};
    if (previous[key] !== undefined && previous[key] !== price) {
      throw cashEconomyError('CASH_PRICE_CONFLICT', '同一日期正式价格不能改写');
    }
    if (previous[key] === price) continue;
    if (!changedDays.has(dateKey)) changedDays.set(dateKey, { ...previous });
    changedDays.get(dateKey)[key] = price;
  }
  if (changedDays.size === 0) return false;
  const days = { ...archive.days };
  for (const [dateKey, prices] of changedDays) days[dateKey] = { prices };
  world.cashPriceArchive = { ...archive, days };
  return true;
}

export function archiveOfficialCashMarket(world, market) {
  return archiveOfficialCashMarkets(world, [market]);
}
