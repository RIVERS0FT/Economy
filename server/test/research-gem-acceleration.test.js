...
test('immediately completes research when less than thirty minutes remain', () => {
  ...
  assert.equal(applyResearchAction(world, user, 'startResearch', { technologyId: 'scientific-agriculture' }, NOW).ok, true);
  ...
  assert.equal(player.research.completedTechnologyIds.includes('scientific-agriculture'), true);
...
test('shortens a six-hour active project by exactly thirty minutes', () => {
  ...
  assert.equal(applyResearchAction(world, user, 'startResearch', { technologyId: 'electronic-engineering' }, NOW).ok, true);
  ...
  assert.equal(player.research.active.technologyId, 'electronic-engineering');
  assert.equal(player.gems, 1);
});
