...
test('market model 14 creates inventory-backed buy and sell orders without system self-trades', () => {
...
test('model 3 migrates directly to model 14 with one-time reserve seeding', () => {
  const world = createWorld(now);
  const player = ensurePlayer(world, alice, now);
  player.credits = 777;
  ...
  assert.equal(world.marketDemand.modelVersion, 14);
...
test('model 5 migrates to model 14 and releases obsolete liquidity reservations', () => {
  const world = createWorld(now);
  const player = ensurePlayer(world, alice, now);
  player.credits = 777;
  player.inventories.wheat.available = 9;
