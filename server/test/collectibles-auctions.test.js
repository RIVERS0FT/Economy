...
test('工厂拍卖冻结运行数量，成交后转移工厂且不写入工厂行情', () => {
  ...
  const lastTradePrice = state.facilityMarkets.farm.lastTradePrice;
  ...
  assert.equal(state.facilityMarkets.farm.lastTradePrice, lastTradePrice, '拍卖成交不得改变最近订单簿成交价');
});
...
test('冻结资产继续计入总资产并提供可用与冻结明细', () => {
  const state = world();
  state.markets.wheat.lastTradePrice = 10;
  state.facilityMarkets ||= {};
  migrateFacilityGroupWorld(state, 1_500);
  state.facilityMarkets.farm.lastTradePrice = 50;
  state.orders.push(
    { id: 'wheat-bid', assetKind: 'commodity', assetId: 'wheat', productId: 'wheat', side: 'buy', ownerType: 'player', ownerId: 2, ownerName: '买家甲', price: 999, quantity: 10, remaining: 10, status: 'open', createdAt: 1_500 },
    { id: 'farm-bid', assetKind: 'facility', assetId: 'farm', facilityTypeId: 'farm', side: 'buy', ownerType: 'player', ownerId: 2, ownerName: '买家甲', price: 999, quantity: 2, remaining: 2, status: 'open', createdAt: 1_501 },
  );
  state.players['1'].inventories.wheat.available = 5;
  state.players['1'].facilityGroups = [{
