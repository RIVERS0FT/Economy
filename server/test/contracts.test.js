...
test('长期合同按批次冻结商品和货款并原子结算，不改变市场价格', () => {
  ...
  assert.equal(applyProductionContractAction(world, supplierUser, 'acceptProductionContract', {
...
test('合同不涉及藏品或工厂，商品不足时进入宽限期并由供应方违约', () => {
  const { world, buyerUser, supplierUser, buyer, supplier, now } = setup();
  ...
  assert.equal(contract.collectibleId, undefined);
...
test('客户端状态包含进行中合同摘要，路由解析所有合同动作', () => {
  ...
  const contract = world.productionContracts[0];
  applyProductionContractAction(world, supplierUser, 'acceptProductionContract', { contractId: contract.id }, now + 1);
