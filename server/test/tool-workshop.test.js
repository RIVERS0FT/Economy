...
test('工具与工具工坊进入正式目录并保持 C4 扣费占款目标', () => {
  ...
  const r = standards(f)[0]; assert.deepEqual(r.inputs, [{ productId: 'steel', quantity: 1 }, { productId: 'lumber', quantity: 1 }]); assert.deepEqual(r.output, { productId: 'tools', quantity: 5 }); assert.equal(r.cycleMs, 60_000); assert.equal(r.operatingCost, 8.05); assert.ok(Math.abs((12 * 5 - 29 - 17 - r.operatingCost) * 60_000 / r.cycleMs - 5.95) < 1e-9);
  assert.deepEqual(f.recipes.map((x) => [x.productionMethodId, x.cycleMs, x.operatingCost, x.output.quantity]), [['forge-working',60000,8.05,5],['precision-machining',30000,10.85,5],['controlled-heat-treatment',90000,4.96,5],['automated-machining',60000,20,10]]);
