...
test('世界版本 25 迁移补齐工具库存与市场且保留资产', () => {
  const now=1786100000000,w=createWorld(now),p=ensurePlayer(w,{id:8,name:'迁移玩家'},now); p.credits=23456;p.inventories.steel.available=11;delete p.inventories.tools;delete w.markets.tools;w.version=24;const m=migrateWorld(w,now+1000);assert.equal(m.version,31);assert.equal(m.players['8'].credits,23456);assert.equal(m.players['8'].inventories.steel.available,11);assert.deepEqual(m.players['8'].inventories.tools,{available:0,frozen:0});assert.equal(m.markets.tools.lastPrice,12);
});
