...
test('manual commodity buys and sells never leave player resting orders or frozen assets', () => {
  ...
  assert.notEqual(buy.executedPrice, 0.01);
  assert.notEqual(sell.executedPrice, 999);
  ...
  assert.ok(Math.abs((seller.credits + sell.fee) - sell.total) < 1e-9);
  ...
  assert.equal(market.todayBuyQuantity + market.todaySellQuantity, 5);
  ...
  assert.equal(audit.soldQuantity + audit.boughtQuantity, market.todayBuyQuantity + market.todaySellQuantity);
