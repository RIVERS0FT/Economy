...
document.documentElement.dataset.appSurface = ['overview', 'map', 'commerce', 'production', 'research', 'contracts', 'auction', 'gem-shop', 'scroll-ownership'].includes(view) ? 'game' : 'auth';
...
function CommerceHarness() {
  ...
  const model = { ...base, game: { ...base.game, lastProcessedAt: fixtureNow, commercialBuildingTypes: types,
    ...
    inventories: { ...base.game.inventories, food: { ...base.game.inventories.food, available: 1 }, beverage: { ...base.game.inventories.beverage, available: 0 } },
  } };
  return <GameShell model={model}><CommercePage model={model} /></GameShell>;
}

const runtimeView = view === 'commerce' ? <CommerceHarness /> : view === 'overview'
    ? <OverviewHarness />
    : view === 'map'
      ? <MapHarness />
    : view === 'production'
