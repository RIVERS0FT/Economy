...
import { acceptServerNow } from '../../src/utils/serverClock.js';
import type { CommodityInvestmentTradeInput } from '../../src/types';
...
const fixedNow = Date.UTC(2026, 8, 11, 4, 0, 0);
document.documentElement.dataset.appSurface = 'game';
acceptServerNow(fixedNow);
...
const baseModel = {
  ...
  game: {
    ...
    saveEpoch: 1,
    economyMode: 'cash',
    commodityInvestmentQuotes: [{ productId: 'wheat', price: 10, available: true, contractId: 'wheat:current',
      priceDateKey: '2026-09-11', expiresAt: Date.UTC(2026, 8, 13, 16), feeBps: 100 }],
    commodityInvestment: { valuationAvailable: true, equity: 200, principal: 180, unrealizedProfit: 20,
      realizedProfit: 0, feesPaid: 0, closeGross: 0, expiryMessage: '', recentTransactions: [], positions: [{
        productId: 'wheat', contractId: 'wheat:current', quantity: 20, cost: 180, averageCost: 9,
        price: 10, value: 200, unrealizedProfit: 20, expiresAt: Date.UTC(2026, 8, 13, 16), status: 'open',
      }] },
    ...
    assetSummary: {
      ...
      investmentValue: 200, workInProgressValue: 50,
      cashValue: 2_200,
...
function Harness() {
  const [userId, setUserId] = useState(77701);
  const [epoch, setEpoch] = useState(1);
  const [game, setGame] = useState(baseModel.game);
  const activeUser = useRef(userId); activeUser.current = userId;
  const requests = useRef<CommodityInvestmentTradeInput[]>([]);
  const notices = useRef<string[]>([]);
  const completion = useRef<((ok: boolean) => void) | null>(null);
  const model = { ...baseModel, user: { ...baseModel.user, id: userId }, game: { ...game, saveEpoch: epoch },
    notify: (message: string) => { notices.current.push(message); document.body.dataset.notices = JSON.stringify(notices.current); },
    tradeCommodityInvestment: async (input: CommodityInvestmentTradeInput) => {
      requests.current.push(input); document.body.dataset.requests = JSON.stringify(requests.current);
      const owner = userId;
      return new Promise<{ ok: boolean; message: string }>((resolve) => {
        completion.current = (ok) => {
          if (ok && activeUser.current === owner) setGame((current) => ({ ...current, credits: current.credits - 10 }));
          resolve({ ok, message: ok ? '测试成交确认' : '测试交易拒绝' });
          completion.current = null;
        };
      });
    },
  } as LoadedGameViewModel;
  Object.assign(window, { investmentHarness: {
    finish: (ok: boolean) => completion.current?.(ok),
    switchUser: () => setUserId((id) => id + 1),
    switchSave: () => setEpoch((value) => value + 1),
    expire: () => setGame((current) => ({ ...current, commodityInvestmentQuotes: current.commodityInvestmentQuotes?.map((q) => ({ ...q, expiresAt: fixedNow - 1 })) })),
    missing: () => setGame((current) => ({ ...current, commodityInvestmentQuotes: current.commodityInvestmentQuotes?.map((q) => ({ productId: q.productId, available: false })) })),
  } });
  return <BankPage model={model} />;
}
