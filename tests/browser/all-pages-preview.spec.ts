...
test('account-free game shell navigates all eleven visible business pages and closes to the map', async ({ page }) => {
  ...
  await expect(provinceTabs.getByRole('tab')).toHaveCount(4);
  await expect(provinceTabs.getByRole('tab', { name: '商业', exact: true })).toHaveCount(0);
...
test('market and building entity lists share surface geometry with registered density exceptions', async ({ page }) => {
  ...
  const densityKeys = new Set<keyof typeof samples[number]>([
    'rowsGap',
    'paddingLeft',
    'paddingRight',
    'paddingTop',
