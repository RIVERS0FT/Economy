...
async function clickMapProvinceLabel(page: import('@playwright/test').Page, provinceName: string) {
  ...
  const point = await label.evaluate((element) => {
    ...
    if (!Number.isFinite(x) || !Number.isFinite(y) || !matrix) throw new Error('province label center transform is missing');
...
test('account-free mode redirects into the complete game shell without API traffic', async ({ page }) => {
  ...
  page.on('request', (request) => { if (request.url().includes('/economy-api')) apiRequests.push(request.url()); });
...
test('player page heading keeps SVG back, centered title, and SVG close in that order', async ({ page }) => {
  ...
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    ...
    const layout = await heading.evaluate((element) => {
      ...
      const box = (target: HTMLElement) => target.getBoundingClientRect();
      return {
        order: children.map((child) => child.classList.contains('page-navigation-button--back') ? 'back' : child.classList.contains('page-heading-title') ? 'title' : child.classList.contains('page-navigation-button--close') ? 'close' : 'unknown'),
        heading: box(element), back: box(children[0]), title: box(children[1]), close: box(children[2]),
        scrollWidth: element.scrollWidth, clientWidth: element.clientWidth,
        padding: [getComputedStyle(element).paddingTop, getComputedStyle(element).paddingRight, getComputedStyle(element).paddingBottom, getComputedStyle(element).paddingLeft],
      };
    });
    ...
    expect(layout.title.left + layout.title.width / 2).toBeCloseTo(layout.heading.left + layout.heading.width / 2, 0);
...
test('overview, market, buildings, and settings share a one-third card width while leaderboard and shop stay full-area', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: '得克萨斯州' })).toBeVisible();
  compactWidths.push((await provinceContent.boundingBox())!.width);
  compactCardWidths.push((await workspaceCard.boundingBox())!.width);

  for (const label of ['概览', '市场', '建筑', '设置']) {
    const button = label === '设置' ? sidebar.locator('.sidebar-footer').getByRole('button', { name: '设置', exact: true }) : sidebar.getByRole('button', { name: new RegExp(`^${label}`) });
    await button.click();
...
test('reduced motion disables card width and page unfold animation', async ({ page }) => {
  ...
  await page.goto('?preview=game');
  await page.locator('.desktop-sidebar').getByRole('button', { name: /^市场/ }).click();
  const transitionDurationSeconds = await page.locator('.signed-in-shell__primary-card').evaluate((element) => Number.parseFloat(getComputedStyle(element).transitionDuration));
...
test('leaderboard and local-only service summaries are populated in the full shell', async ({ page }) => {
  ...
  for (const viewport of [{ width: 900, height: 900 }, { width: 390, height: 844 }]) {
    ...
    const geometry = await leaderboardSwitch.evaluate((element) => ({
      rows: [...element.querySelectorAll<HTMLElement>('button')].map((button) => button.getBoundingClientRect().top),
      hasHorizontalOverflow: element.scrollWidth > element.clientWidth + 1,
    }));
    expect(Math.max(...geometry.rows) - Math.min(...geometry.rows)).toBeLessThanOrEqual(1);
