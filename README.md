...
用户可见的成交记录、资金变化、商品变化、工厂变化和生产变化只保存在当前浏览器。
...
浏览器本地存储、客户端时间和客户端计算不能改变正式资金、库存、工厂、生产计划、订单、产权或排名。
...
旧服务器日志会在下一次读取并保存时从世界 JSON 中清除，但不会改变玩家资金、库存、工厂、订单、挂牌或市场状态。
