...
- 建设新工厂卡在桌面生产管理区独占左侧列并使用 `sticky` 常驻；移动端恢复普通文档流。
- 工厂卡桌面固定高度，开关固定在右上角；异常原因直接合并为“异常：仓库已满／资金不足／原料不足”等顶部状态。
- 工厂数量只在标题和顶部摘要展示“运行中／下一周期加入／冻结中”，不再显示持有卡片或下一周期产量说明。
- 生产计划使用“当前计划 + 下拉框 + 可选数量输入 + 保存计划”的单行结构；定量生产完成后服务器关闭运行开关。
- 共享仓库允许无限扩容，当前等级升级容量增量为 `250 + 50 × (等级 - 1)`，升级费用保持 `150 × 等级²`。
- 客户端状态版本为 11，世界状态版本为 7。
