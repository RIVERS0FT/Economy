...
- 当前目录共 13 种商品和 12 种工厂类型；主食细分为小麦和水稻，食品属于加工后的饮食替代品。
- 产业链包括 `小麦 → 面粉 → 食品`、`木材 → 木板 → 家具`、`铁矿石 → 钢材 → 机械` 和 `原油 → 塑料 → 电子产品`。
- 食品、小麦和水稻共享同一个人口饮食需求组，每 5 分钟最多 330 货币预算；小麦和水稻每件消费效用为 1，食品为 3，按效用调整后的订单簿价格竞争。食品最多获得 80% 预算，小麦和水稻各最多获得 50%，未使用预算不结转，食品不得再获得独立人口需求。
- 商品初始参考价、生产数量、周期秒数和周期成本全部保持整数；当前平衡目标为基础原料 1／分钟、中间产品 3／分钟、最终产品 6／分钟。
- 农场的种植小麦和种植水稻配方保持同参数：周期 120 秒、单座产量 4、单座周期成本 6；农作物是长耗时、低收益的产业起点。
- 矿场仍只生产铁矿石，钢铁厂仍只将铁矿石加工为钢材；数值可以统一平衡，但不得增加本轮未设计的新商品、配方或改名。
...
- Economy 注册完成时点是统一账号第一次创建 Economy 玩家档案；已登录主页账号首次进入仍可自动建档并记录 IP 指纹，邮箱验证码注册使用同一首次建档与多账号规则。
