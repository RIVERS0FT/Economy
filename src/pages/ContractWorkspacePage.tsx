...
function ActiveContractCard({ contract, label, busy, run }: { contract: ProductionContract; label: string; busy: boolean; run: RunAction }) {
  ...
  return (
    <PagePanel className={className}>
      <header className="contract-card-heading"><div><div className="contract-card-tags"><StatusTag tone={statusTone(contract)}>{STATUS_LABELS[contract.status]}</StatusTag><StatusTag>{roleTag(contract)}</StatusTag></div><h2>{label}</h2><p>{contract.issue || `下一状态边界：${dateTimeLabel(contract.nextDueAt)}`}</p></div></header>
...
function HistoryRow({ contract, label, republish }: { contract: ContractAuditHistoryItem; label: string; republish: () => void }) {
  return <article className="contract-history-result-grid"><div><StatusTag>{STATUS_LABELS[contract.status]}</StatusTag><strong>{label}</strong><span>{dateTimeLabel(contract.endSummary?.endedAt || contract.endedAt)}</span></div><DataList className="compact"><DataRow label="累计结算" value={<CurrencyAmount>{formatCurrency(contract.grossTotal)}</CurrencyAmount>} /><DataRow label="累计商品" value={<CompactNumber value={contract.transferredGoods} />} /></DataList><Button variant="text" onClick={republish}>重新拟定</Button></article>;
}
          ...
          <section className={`contract-personal-content${personalView === 'active' ? ' contract-active-grid' : ''}`} role="tabpanel" tabIndex={0}>
            {personalView === 'active' ? (activeContracts.length ? activeContracts.map((contract) => <ActiveContractCard key={contract.id} contract={contract} label={labelFor(contract)} busy={Boolean(busyKey)} run={run} />) : <EmptyState>当前没有进行中的合同。</EmptyState>) : (
              <PagePanel className="contract-history-panel">
                ...
                {historyItems.map((contract) => <HistoryRow key={contract.id} contract={contract} label={contract.kind === 'supply' ? products.get(contract.productId) || contract.productId : contract.kind === 'loan' ? '玩家抵押借贷' : '工厂租赁'} republish={() => { setRepublish(contract); setShowPublish(true); model.notify('已填入历史合同参数，请核对当前条件后发布。'); }} />)}
