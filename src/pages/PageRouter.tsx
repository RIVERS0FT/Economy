...
function cachedLoader<T>(loader: () => Promise<T>) {
  ...
  return () => { promise ??= loader(); return promise; };
}
...
const pagePreloaders: Record<TabId, () => Promise<unknown>> = {
  home: loadOverviewPage, map: loadMapPage, province: loadProvincePage, market: loadGlobalMarketPage, buildings: loadGlobalBuildingsPage,
  transport: loadTransportPage, research: loadResearchPage, auction: loadAuctionPage, contracts: loadContractPage, bank: loadBankPage,
  leaderboard: loadLeaderboardPage, 'gem-shop': loadGemShopPage, settings: loadSettingsPage,
};
...
export function preloadPage(tab: TabId) { return pagePreloaders[tab](); }
...
function AuthorityPageBoundary({ model, dependencies, render }: { model: OnlineAutoTradeAwareGameViewModel; dependencies: readonly StateAuthorityDependency[]; render: () => ReactNode }) {
...
export function PageRouter({ model }: { model: OnlineAutoTradeAwareGameViewModel }) {
  ...
  switch (tab) {
    case 'map': renderPage = () => <MapPage model={model} />; break;
    case 'province': renderPage = () => <ProvincePage model={model} />; break;
    case 'market': renderPage = () => <GlobalMarketPage model={model} />; break;
    case 'buildings': renderPage = () => <FacilityRecipeProfitMarketsProvider markets={model.game.markets}><GlobalBuildingsPage model={model} /></FacilityRecipeProfitMarketsProvider>; break;
    case 'transport': renderPage = () => <TransportPage model={model} />; break;
    case 'research': renderPage = () => <ResearchPage model={model} />; break;
    case 'auction': renderPage = () => <AuctionPage model={model} />; break;
    case 'contracts': renderPage = () => <ContractPage model={model} />; break;
    case 'bank': renderPage = () => <BankPage model={model} />; break;
    case 'leaderboard': renderPage = () => <LeaderboardPage model={model} />; break;
    case 'gem-shop': renderPage = () => <GemShopPage model={model} />; break;
    case 'settings': renderPage = () => <SettingsPage model={model} />; break;
    case 'home': default: renderPage = () => <OverviewPage model={model} />;
  }
  return (
    <Suspense fallback={<div className="page-content page-loading" role="status">正在加载页面…</div>}>
      <AuthorityPageBoundary key={tab} model={model} dependencies={PAGE_AUTHORITY_DEPENDENCIES[tab]} render={renderPage} />
    </Suspense>
  );
}
