...
export function AdminPopulationHealth({ economy }: { economy: PopulationEconomyAdminSummary }) {
  ...
  const sourceRows = [
    ...
    { label: '银行服务', value: economy.sources.banking },
    { label: '研发服务', value: economy.sources.research },
  ];
  ...
  const totalLaborForce = models.reduce((sum, model) => sum + model.laborForce, 0);
  const totalEmployed = models.reduce((sum, model) => sum + model.employed, 0);
  const totalUnemployed = models.reduce((sum, model) => sum + model.unemployed, 0);
  const totalVacancies = models.reduce((sum, model) => sum + model.vacancies, 0);
  const employmentRateBps = totalLaborForce > 0 ? Math.round(totalEmployed / totalLaborForce * 10_000) : 0;
  const migrationLabel = demographics.lastMigrationDirection === 'in'
    ? `迁入 ${formatNumber(demographics.lastMigration)}`
    : demographics.lastMigrationDirection === 'out'
      ? `迁出 ${formatNumber(demographics.lastMigration)}`
      : '无迁移';
  const capacityRows = Object.entries(demographics.structuralCapacityByComplexity);
