...
export function CommercialBuildingDetail({ group, type, products, inventories, inventoryFreezeDetails, markets, now, pending, onToggle,
  ...
  const money = (value: number | null) => value === null ? '—' : <CurrencyAmount>{formatCurrency(value)}</CurrencyAmount>;
  return (
    <>
      ...
      <BuildingSettlementPanel className="commercial-settlement" title={<GameConcept concept="commercial-settlement">经营结算</GameConcept>}
        status={group.status} description={`${type.name}经营结算，${settlement.locked ? '本周期结果' : '下一周期预计结果'}，参与 ${settlement.count} 座，收入 ${settlement.revenue === null ? '待确认' : formatCurrency(settlement.revenue)}`}
