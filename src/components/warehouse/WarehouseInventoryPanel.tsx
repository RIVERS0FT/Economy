...
export function WarehouseInventoryPanel({
  ...
  const openAutoTradePanel = useCallback((
    ...
    if (isMobileWarehouseLayout()) setMobileAutoTradeOpen(true);
  ...
  return (
    <>
      <div className={`production-warehouse-workspace ${className}`.trim()}>
        <PagePanel className="production-surface warehouse-auto-trade-card">
          <WidgetHeading
          ...
          <section className="warehouse-content" aria-label="仓库内容">
            ...
            {stockedProducts.length > 0 ? (
              <div className="warehouse-product-grid">
                {stockedProducts.map((product) => {
                  ...
                  return (
                    <button
                      type="button"
                      className={`warehouse-product-card ${automationLabel ? 'is-auto-trade-enabled' : ''}`}
              ...
              <div className="empty-state warehouse-content-empty">
                ...
                <span>可以点击“自动交易”为零库存商品开启自动采购。</span>
              </div>
            )}
          </section>
        </Panel>
