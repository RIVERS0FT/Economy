export const GAME_CONCEPTS = {
  ...
  'transport-fuel': {
    label: '每趟燃料',
    description: '全线距离乘以运输方式的每公里耗油量和本趟出车数量，整趟只向上取整一次。从起点可用库存一次性扣除燃料商品，途中不重复扣除、不自动购油。动力燃料不占载荷，作为货物运输的燃料仍占载荷。',
  },
  ...
  'transport-fleet': {
    label: '路线运力',
    description: '每条路线初始包含一个载具，支持批量增购，最多拥有 100 个。公路每辆载荷 200、铁路每列 2,000、航空每架 500。系统按整趟货源、资金、燃料和增益自动选择出车数量，未派出的载具不收运行费；增购不加速，新增运力从下一趟可用，本趟容量与费用保持不变。',
  },
  'transport-route-maintenance': {
    label: '路线维护',
    description: '路线创建后可重命名或增加运力，不能修改路径、站点或运输方式；需要调整时必须重新创建。运行中可预约本趟完成后删除，先返回并卸完货，再删除且不启动下一趟；删除同时移除全部附属载具，建线费和增购费用均不退还。',
  },
