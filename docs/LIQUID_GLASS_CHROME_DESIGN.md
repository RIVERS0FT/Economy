...
| `src/styles/mobile-status-layout.css` | 移动安全区、顶部状态栏固定网格和状态项紧凑布局 |
| `src/app/GameApp.tsx` | 状态项完整值与移动紧凑值 |
...
- 使用 `repeat(4, minmax(0, 1fr))` 四等分网格，所有项目必须在当前视口内展示；
- 顶部状态栏必须设置 `overflow-x: hidden`，不得恢复横向滚动、滚动吸附或按内容宽度自动扩列；
- 每个状态项使用 `min-width: 0`，由图标和紧凑值共同占用等分空间；
- 可用资金、总资产和仓库剩余在移动端使用 K/M/B/T 紧凑数字，桌面继续显示完整数值；
- 排名在移动端使用 `#1`、`#2` 格式，桌面继续使用“第 1 名”；
- 紧凑值必须保持可读，不得通过扩大状态栏高度、隐藏整个状态项或恢复横向滚动解决宽度问题。
...
9. 移动资金和资产显示紧凑数值，排名显示为 `#N`。
10. 不支持模糊时仍有清晰、可读的回退表面。
11. 页面滚动、表格滚动和移动安全区不受影响。
12. 移动底部导航选中、悬停或触摸后按钮与图标均不发生位移或缩放。
13. 完整构建和液态玻璃架构检查通过。
