...
- 战略地图固定使用唯一静态 SVG 世界面，由“世界大陆海岸轮廓背景 + 美国本土连续 48 州经营层 + 州名／运输叠层”组成。美国本土连续 48 州是唯一经营和交互地理；世界其他地区只能显示低对比大陆／海岸轮廓，不显示国家、州省或其他行政边界、名称、填充、Tooltip、焦点、选中、点击或运输路线选点能力，世界背景必须保持 `pointer-events:none` 且从可访问交互树隐藏。不得为了世界背景建立第二张地图、第二个相机、第二套经营地区数据或服务器状态。
- 战略地图滚轮与双指逻辑缩放继续允许 `0.5–4`，默认 `0 / 0 / 1` 镜头仍是美国本土完整轮廓的等比 Contain 视图。世界背景、48 州、中文州名与运输叠层共用同一个静态 SVG 世界坐标系和单一合成相机；缩放、平移每帧最多一次写入 `.province-map-camera-surface` 的 `translate3d + scale`，世界轮廓和州面 path `d`、州名基础坐标及 glyph transform 在手势期间保持不变。根级 `.application-map-layer` 继续承担最终物理视口裁剪，屏幕外的州面与世界背景几何不得从世界面卸载。滚轮／双指围绕真实焦点更新 `x / y / zoom`，同一帧输入合并；鼠标／单指拖动和双指平移直接修改同一个目标相机。每次拖动、滚轮缩放或双指缩放更新目标后，都必须依据世界投影边界、当前容器尺寸、SVG `viewBox` 与当前 zoom 在写入下一帧前连续 clamp `x / y`；不得先越界再在手势结束后回弹。边界只允许约 `12px` 的视觉安全留白；若任一轴在当前缩放下小于可视区域，则该轴必须居中并禁止继续平移，禁止使用固定屏幕分辨率的硬编码 `xMin/xMax/yMin/yMax`。只要仍有待提交的 `requestAnimationFrame` 相机写入，本轮输入就不得先进入 idle，必须至少提交一帧 active 合成结果后才允许 `INPUT_SETTLE_MS` 收口；`will-change: transform` 只在 active 期间临时开启并在停止后清除。地图拖动、缩放不得调用 ECharts、ZRender、重新投影、重新布局州名或维护第二套标签相机；空白双击／双触继续恢复默认美国 Contain 镜头。移动双指从州面、州界附近或地图空白起手必须等价；一旦本轮出现双指，从多点手势开始到最后触点抬起后的 `420ms` 内，必须抑制合成 click、州面选择和空白双触重置，窗口结束后的正常单指点击立即恢复。多点抑制只负责输入仲裁，不得驱动、回滚或复制相机。
...
- `tests/browser/province-map-world-boundary.spec.ts`：锁定世界背景只有一条不可交互的大陆／海岸轮廓路径、美国本土 48 州是唯一可交互地理，并验证相机在连续拖动后稳定停在世界边界、继续同向拖动不能进入无限空白。
