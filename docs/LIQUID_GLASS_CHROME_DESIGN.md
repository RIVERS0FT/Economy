...
- “公开经济事件”直接读取 `economicCalendar.events`。正在生效与未来七天已公布事件使用紧凑行，行内只显示名称、状态点和距离开始／结束时间；点击详情的内容归 `PAGE_CONTENT_AND_NAVIGATION_DESIGN.md`，不在追踪器内展开第二套详情。已经结束的最近事件收进“最近结束”折叠组，不得继续用每项至少 `4rem` 的独立大卡或事件列表专属滚动区。事件日历的服务器窗口、预算语义与成交反馈权威规则保持不变。
