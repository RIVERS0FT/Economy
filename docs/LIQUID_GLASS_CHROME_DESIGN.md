...
移动底部导航活动状态不得改变按钮或图标位置；同时使用紧凑但仍可触控的几何尺寸：

- 底栏高度固定为 `68px`，面板垂直内边距为 `.3rem`；
- 导航按钮固定高度为 `50px`、最小宽度为 `60px`，不得低于 `44px` 的触控下限；
- 导航按钮圆角为 `.85rem`，内边距为 `.16rem .25rem .22rem`；
...
15. 移动底栏高度为 `68px`，内部导航按钮为 `60px × 50px` 的最小几何尺寸。
16. 移动导航图标与文字紧凑排列，图文组垂直居中且按钮没有被拉伸到整栏高度。
