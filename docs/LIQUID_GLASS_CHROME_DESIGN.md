...
| `src/styles/mobile-status-layout.css` | 移动安全区、顶部状态栏全宽紧凑 Flex 布局和状态项样式 |
...
- 容器使用紧凑 Flex，并通过左右安全区变量同时设置 `left` 和 `right`，铺满当前可用屏幕宽度；
- 容器使用 `width: auto`、`max-width: none` 和 `transform: none`，不得恢复内容宽度胶囊或水平位移居中；
- 内部状态项整体使用 `justify-content: center` 和小范围响应式 `gap` 保持紧凑，不得使用 `space-between` 拉开项目；
- 状态项使用 `flex: 0 0 auto`，不得使用 `flex: 1`、四等分网格或按内容宽度自动扩列；
...
- 状态项之间不增加分隔线，分组只通过图标与数值的小间距、项目之间的较大间距表达；
...
9. 状态栏铺满左右安全区之间的可用宽度，内部四项仍保持紧凑居中。
10. 四个 SVG 图标视觉尺寸、基线和描边一致，移动显示尺寸不低于 `18px`。
11. 移动状态项之间没有分隔线、独立边框或图标底板。
12. 移动资金和资产显示紧凑数值，排名显示为 `#N`。
13. 不支持模糊时仍有清晰、可读的回退表面。
14. 页面滚动、表格滚动和移动安全区不受影响。
15. 移动底部导航选中、悬停或触摸后按钮与图标均不发生位移或缩放。
16. 完整构建和液态玻璃架构检查通过。
