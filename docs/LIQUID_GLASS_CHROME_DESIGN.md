...
| `src/styles/mobile-status-layout.css` | 移动安全区、顶部状态栏紧凑 Flex 胶囊和状态项布局 |
...
- 容器使用紧凑 Flex，宽度由内容决定，通过 `left: 50%` 与 `translateX(-50%)` 居中；
- 容器使用 `width: max-content`，同时受左右安全区计算出的 `max-width` 限制；
- 使用 `justify-content: center` 和小范围响应式 `gap`，不得使用 `space-between` 拉开项目；
