...
| `src/styles/mobile-status-navigation.css` | 移动端尺寸、横向滚动、导航图文间距和交互，不重复定义玻璃材质 |
...
移动底部导航活动状态不得改变按钮或图标的几何位置，图标与文字必须保持紧凑：
...
- 移动导航按钮使用 `grid-template-rows: 1.5rem min-content`，图标行不得恢复为会产生额外留白的 `1.85rem`；
- 移动导航按钮使用 `align-content: center`，图标与文字作为一个整体在按钮中垂直居中，不允许 `auto` 行拉伸放大视觉间距；
- 图标行与文字行的 CSS 间距固定为 `gap: .08rem`，不得恢复 `.16rem` 或更大的图文间距；
...
15. 移动底部导航图标与文字紧凑排列，CSS 行间距为 `.08rem`，且整个图文组垂直居中。
16. 不支持模糊时仍有清晰、可读的回退表面。
17. 页面滚动、表格滚动和移动安全区不受影响。
18. 移动底部导航选中、悬停或触摸后按钮与图标均不发生位移或缩放。
19. 完整构建和液态玻璃架构检查通过。
