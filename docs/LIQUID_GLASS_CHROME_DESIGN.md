...
- 正式页面 ID 发生变化时，桌面玩家侧栏的展开状态必须立即恢复为 `78px` 收起态，并重新建立本轮指针意图门槛；页面切换不得继承上一页的 `224px` 展开状态，静止在侧栏上的旧指针位置也不得让新页面自动重新展开。切页后的首次真实 `pointermove`／`mousemove` 才重新允许悬浮展开；键盘焦点进入仍立即展开。
...
- 战略地图滚轮与双指逻辑缩放继续允许 `0.5–4`，交互帧只允许地图 SVG 与中文州名 SVG 使用同一临时 CSS 矩阵；缩放 settle 后必须把该矩阵等价提交到地图 `layoutSize + layoutCenter`，同时保持 map series `zoom = 1`，使州填充、州界、坐标系和 ECharts 内部绘制／裁剪边界同步缩放。不得恢复以 `geoRoam.zoom` 作为持久缩放提交，也不得通过放开根级 `.application-map-layer` 的视口裁剪掩盖边界问题；地图拖动继续由 `roam: 'move'` 处理，空白双击／双触继续恢复 contain 镜头。
...
- `scripts/verify-sidebar-navigation-collapse.mjs`：锁定正式页面切换强制恢复 `78px` 收起态、按页面重置真实指针意图门槛以及专项浏览器回归，禁止恢复跨页面继承侧栏展开状态。
...
- `tests/browser/sidebar-navigation-collapse.spec.ts`：桌面侧栏展开后切换正式页面必须立即回到 `78px` 收起态，静止旧指针不得重新展开；切页后的首次真实指针移动恢复 hover 展开，后续键盘焦点进入仍可展开。
...
- `tests/browser/map-zoom-out-boundary.spec.ts`：地图缩小到 `0.5` 后，加利福尼亚、佛罗里达、缅因和华盛顿等外围州的州名中心仍必须命中对应可见州面，持久缩放不得因内部裁剪边界未同步而让外围州填充／州界消失。
