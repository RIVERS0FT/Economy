...
- 战略地图固定使用唯一静态 SVG 世界面，由“10m 世界大陆背景 + 美国本土连续 48 州经营层 + 州名／运输叠层”组成。世界大陆背景固定使用 `world-atlas@2.0.2` 的 Natural Earth 1:10m countries 数据，并只把可能进入美国有限平移上下文的北美陆地对象 dissolve 为单一无行政边界 path，提供低对比土地填充、海岸主线和柔和外阴影；不得把完整全球 10m geometry 送入常驻 SVG，否则会把不可见地理复杂度带入所有玩家页面的指针热路径；美国本土连续 48 州继续使用 `us-atlas@3.0.1` 的 `states-10m`，并把这 48 个州在同一 TopoJSON 中 dissolve 成美国外轮廓作为最终可见美国边线。美国外轮廓必须覆盖世界 land 在美国沿海的对应描边，使美国海岸与州外边界只出现一条可见 10m 级边线，不得同时暴露两套不同来源或不同精度的双线。美国本土连续 48 州仍是唯一经营和交互地理；世界其他地区不得显示国家、州省或其他行政边界、名称、Tooltip、焦点、选中、点击或运输路线选点能力，世界背景必须保持 `pointer-events:none` 且从可访问交互树隐藏。大陆外阴影只用于低强度海岸层次，强度必须弱于州选中辉光；不得为了世界背景建立第二张地图、第二个相机、第二套经营地区数据或服务器状态。
...
- `tests/browser/province-map-world-boundary.spec.ts`：锁定世界背景只 dissolve 可进入有限镜头的北美 10m 陆地上下文，提供大陆填充、海岸主线和低强度外阴影且保持完全不可交互；美国外轮廓由同一 `states-10m` 州拓扑合并生成并覆盖世界 coastline 的美国对应描边；桌面最小逻辑 `1×` 时美国本土约占有效地图面积 `2/3` 并居中且不可平移，放大后平移只在逐步扩展的美国周边上下文内连续 clamp，竖屏优先完整显示并居中。
