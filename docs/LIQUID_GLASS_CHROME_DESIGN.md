...
- 正式页面 ID 变化只切换页面内容，不得写入桌面玩家侧栏的展开状态，也不得重置本轮指针或前台交互意图。只要指针仍悬浮在侧栏内，或键盘焦点仍位于侧栏内，切换概览、市场、建筑、研发、设置等正式页面后必须继续保持 `224px` 展开态；只有真实 `mouseleave`、焦点离开侧栏、浏览器失焦／后台等侧栏自身生命周期事件才允许恢复 `78px`。页面切换和侧栏输入状态必须保持彼此独立。
...
- 战略地图固定使用唯一静态 SVG 世界面，由“10m 世界大陆背景 + 美国本土连续 48 州经营层 + 州名／运输叠层”组成。世界大陆背景的源数据固定为 `world-atlas@2.0.2` 的 Natural Earth 1:10m countries 数据；仓库内必须由确定性生成脚本先 dissolve 可能进入美国有限平移上下文的北美陆地，再只保留该结果引用的 TopoJSON arcs 并重映射为裁剪后的单一运行时 topology，客户端只允许导入这份裁剪 topology 并展开为无行政边界几何，用于低对比土地填充、海岸主线和柔和外阴影。完整全球 10m atlas 只能存在于生成／防回退工具输入，禁止被玩家运行时模块直接 import、解析或打入常驻 `GameApp` 包；也禁止把裁剪结果预展开成高体积浮点 GeoJSON，因为这会抵消 TopoJSON 的共享弧与量化压缩收益；美国本土连续 48 州继续使用 `us-atlas@3.0.1` 的 `states-10m`，并把这 48 个州在同一 TopoJSON 中 dissolve 成美国外轮廓作为最终可见美国边线。美国外轮廓必须覆盖世界 land 在美国沿海的对应描边，使美国海岸与州外边界只出现一条可见 10m 级边线，不得同时暴露两套不同来源或不同精度的双线。美国本土连续 48 州仍是唯一经营和交互地理；世界其他地区不得显示国家、州省或其他行政边界、名称、Tooltip、焦点、选中、点击或运输路线选点能力，世界背景必须保持 `pointer-events:none` 且从可访问交互树隐藏。大陆外阴影只用于低强度海岸层次，强度必须弱于州选中辉光；不得为了世界背景建立第二张地图、第二个相机、第二套经营地区数据或服务器状态。
- 战略地图滚轮与双指逻辑缩放固定为 `1–4`；逻辑 `1×` 是按当前真实地图视口和美国本土投影包围盒动态计算的最小基础镜头，不等价于固定 CSS `scale(1)`。桌面与横屏在不裁掉美国的前提下，使美国本土包围盒面积目标约占有效地图视口的 `2/3`，且包围盒中心必须与真实地图视口中心重合；窄屏／竖屏若强制 `2/3` 会裁掉美国，则完整显示连续 48 州和约 `12px` 安全边距优先，仍保持居中，允许实际面积比例低于 `2/3`。逻辑 `1×` 时两个轴都锁定在该居中位置而不可平移；从 `1×` 放大到 `4×` 时，合法平移上下文随缩放连续扩展，最大只允许围绕美国本土包围盒向水平方向约扩展 `35%`、垂直方向约扩展 `25%`，不得把镜头拖到远离美国的全球区域。世界背景、48 州、中文州名与运输叠层共用同一个静态 SVG 世界坐标系和单一合成相机；缩放、平移每帧最多一次写入 `.province-map-camera-surface` 的 `translate3d + scale`，世界轮廓和州面 path `d`、州名基础坐标及 glyph transform 在手势期间保持不变。根级 `.application-map-layer` 继续承担最终物理视口裁剪，屏幕外的州面与世界背景几何不得从世界面卸载。滚轮／双指围绕真实焦点更新同一个目标相机，鼠标／单指拖动和双指平移也只修改该目标；每次输入更新后必须在写入下一帧前依据当前逻辑缩放、美国本土上下文包围盒、容器尺寸与 SVG `viewBox` 连续 clamp，不得先越界再回弹。若任一轴的合法上下文小于可视区域，该轴必须居中并禁止继续平移，禁止使用固定屏幕分辨率的硬编码边界。空白双击／双触、真实容器 resize 的地图重置都必须回到动态计算后的逻辑 `1×` 美国居中镜头。只要仍有待提交的 `requestAnimationFrame` 相机写入，本轮输入就不得先进入 idle，必须至少提交一帧 active 合成结果后才允许 `INPUT_SETTLE_MS` 收口；唯一 `.province-map-camera-surface` 是长期合成相机层，必须常驻 `will-change: transform`，不得在每轮手势开始／结束时反复切换该属性。相机 `requestAnimationFrame` 热路径除内存相机状态外只允许一次 `translate3d + scale` 写入，诊断 `data-*` 只允许在初始化、输入开始、输入结束或重置等边界同步，不得逐帧批量写入；输入 settle 只能更新一个 deadline 并复用单一定时器，不得为每个 wheel／pointermove 反复 clear／创建 timer。地图拖动、缩放不得调用 ECharts、ZRender、重新投影、重新布局州名或维护第二套标签相机。移动双指从州面、州界附近或地图空白起手必须等价；一旦本轮出现双指，从多点手势开始到最后触点抬起后的 `420ms` 内，必须抑制合成 click、州面选择和空白双触重置，窗口结束后的正常单指点击立即恢复。多点抑制只负责输入仲裁，不得驱动、回滚或复制相机。在途运输位置的 `500ms` 共享时间只允许由运输 marker 最小动态子树消费，运输 Tooltip 只在实际打开时可作为独立叶子消费同一共享时间；`StrategicMapStage` 不得订阅该高频时间，时间 tick 不得因此重新提交世界背景、48 州、州名、运输路线或相机根。
- 战略地图州面交互固定采用“镜头底色 + 中性轮廓”分层，参考大战略地图的悬浮与选中层级但保持 Economy 自身配色。每个州的基础 `areaColor` 继续由当前政治／资产／工业／市场／异常镜头决定；桌面悬浮、选中以及选中后继续悬浮都必须原样复用该州当前 `areaColor`，不得再用 `--color-surface-hover`、`--color-success-strong` 或其他业务状态色覆盖镜头底色。普通悬浮只使用 `--color-text-secondary` 的 `1.5px` 中性亮边，不增加辉光；选中使用 `--color-text-primary` 的 `2.5px` 亮边与低强度 `5px` 辉光；已选中州再次悬浮时提升为 `3px` 亮边与 `7px` 辉光，视觉强度必须保持“选中悬浮 > 选中 > 普通悬浮 > 默认”。连续 48 州在任何交互状态下都保持对应镜头的正常底色；鼠标悬浮其他州不得清除当前选中州。上述视觉只能通过同一静态 SVG path 的原生 CSS `:hover`／`:focus-visible` 与外部 `data-selected` 状态实现，不得增加第二张地图、第二套州面 SVG、ECharts `emphasis/select`、pointermove 驱动的 React 高频视觉状态或交互时 `setOption` 重绘；州名选中仍只更新既有 `data-selected`，不得触发完整标签布局，也不得修改合成相机。
...
- `tests/browser/province-map-world-boundary.spec.ts`：锁定世界背景使用预生成的北美 10m 裁剪 TopoJSON，提供大陆填充、海岸主线和低强度外阴影且保持完全不可交互；`scripts/verify-provincial-economy.mjs` 同时锁定该 topology 可由固定 `world-atlas@2.0.2` 输入逐字重建、运行时文件体积保持受限，且玩家模块不得直接 import 完整 atlas；美国外轮廓由同一 `states-10m` 州拓扑合并生成并覆盖世界 coastline 的美国对应描边；桌面最小逻辑 `1×` 时美国本土约占有效地图面积 `2/3` 并居中且不可平移，放大后平移只在逐步扩展的美国周边上下文内连续 clamp，竖屏优先完整显示并居中。
- `tests/browser/map-zoom-out-boundary.spec.ts`：必须先把地图放大到外围州真实离开屏幕，再缩小到逻辑 `1×`，验证在合成相机仍 active 时加利福尼亚、佛罗里达、缅因和华盛顿已经重新进入物理视口且州名中心命中对应州面；`tests/browser/map-zoom-transient.spec.ts` 锁定手势期间 48 条 path `d` 与 glyph 基础 transform 不变、同帧滚轮只写一次合成相机且 active 热路径不批量写诊断属性；`tests/browser/map-zoom-render-sync.spec.ts` 锁定州面和州名共享同一个静态世界与相机；`tests/browser/map-reset-sync.spec.ts` 锁定空白重置首帧同步到动态逻辑 `1×` 美国居中镜头。
