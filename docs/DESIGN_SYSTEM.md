...
- 顶部状态栏使用纯色背景 `--mobile-chrome-surface`，不得使用透明度渐变或线性渐变。
...
4. 顶部状态栏是否仍为纯色背景且没有透明度渐变；
5. 顶部状态项是否仍为“图标 + 数字在上、文字在下”的紧凑布局；
6. 顶部状态栏是否仍隐藏第三行补充信息；
7. 底部导航是否仍为纯色背景且没有透明度渐变；
8. 导航栏是否保留左右内边距；
9. 图标和文字是否保持放大后的尺寸；
10. 选中项是否仍使用纯色半透明背景、边框和图标高亮；
11. 上下栏四边边框和圆角弧线是否完整；
12. 非页面滚动容器是否仍在边界释放滚动手势；
13. 最后一张内容卡片是否能完整滚动到导航栏上方；
14. 是否同步更新了本设计文档。
