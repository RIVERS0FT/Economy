...
- 顶部状态栏和底部导航栏都使用统一的半透明纯色背景 `--mobile-chrome-glass-surface`。
- 半透明纯色必须配合 `backdrop-filter` 和 `-webkit-backdrop-filter` 的模糊与饱和度效果形成玻璃质感。
- 上下栏不得使用透明度渐变或线性渐变。
- 不得改回完全不透明的 `--mobile-chrome-surface`，否则后方内容无法参与混合，玻璃效果会消失。
- 不得在移动端最终样式中保留 `backdrop-filter: none`。
...
- 移动端状态栏、导航栏、玻璃效果与滚动释放规则：`src/styles/mobile-status-navigation.css`
...
4. 上下栏是否仍使用统一半透明纯色而不是透明度渐变；
5. `backdrop-filter` 和 `-webkit-backdrop-filter` 是否仍启用；
6. 顶部状态项是否仍为“无底座图标 + 数字在上、文字在下”的紧凑布局；
7. 顶部状态栏是否仍隐藏第三行补充信息；
8. 底部导航图标是否仍无独立底座；
9. 导航栏是否保留左右内边距；
10. 图标和文字是否保持放大后的尺寸；
11. 选中项是否仍使用按钮级纯色半透明背景、边框和图标高亮；
12. 上下栏四边边框和圆角弧线是否完整；
13. 非页面滚动容器是否仍在边界释放滚动手势；
14. 最后一张内容卡片是否能完整滚动到导航栏上方；
15. 是否同步更新了本设计文档。
