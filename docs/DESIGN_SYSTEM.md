...
- 底部导航在有系统安全区时使用 `max(--mobile-chrome-inset, env(safe-area-inset-bottom))`。
...
- 顶部和底部必须使用完全一致的背景、模糊、边框和阴影参数。

### 2.3 顶部状态栏

- 顶部状态栏只能有一个液态玻璃外层，不得在内部继续嵌套信息卡片。
- 状态项直接排列在玻璃表面上，采用“无底座图标 + 数字或文字”的单行水平结构。
- `.asset-bar-item` 不得使用独立背景、边框、圆角或阴影。
- 顶部图标不得使用独立边框、圆角或背景底座。
- 状态名称和辅助信息在移动端不占用可见空间，由状态项的可访问名称保留完整语义。
- 移动端核心值不得重复显示已经由图标表达的货币或分类符号，例如货币图标后不得再次显示同一个货币符号。
- 桌面端完整值和移动端紧凑值通过 `value` 与 `compactValue` 分离，不能为修复移动端而删除桌面端单位。
- 主要资产和市场信息只能通过颜色、字重或字号强调，不得重新添加背景块或边框。
- 状态项图标必须由 React 数据显式传入，不得使用 CSS `nth-child` 根据排列顺序生成。
- 状态栏使用内容宽度横向排列；宽度不足时横向滚动，不得压缩或截断核心数字和文字。
...
- 移动端业务页面响应式规则：`src/styles/mobile-pages.css`
...
6. 上下栏是否没有透明度渐变；
7. 顶部状态栏是否只有一个液态玻璃外层；
8. `.asset-bar-item` 是否没有背景、边框、圆角和阴影；
9. 顶部状态是否仍为“无底座图标 + 数字或文字”的单行结构；
10. 移动端紧凑值是否没有重复货币或分类图标；
11. 主要状态是否只通过内容颜色、字重或字号强调；
12. 顶部状态图标是否由数据显式提供且没有 CSS 顺序依赖；
13. 底部导航卡片是否没有固定左右 padding；
14. 左右滚动留白是否仍由首尾伪元素提供；
15. 左右留白是否会跟随按钮滚动，并在滚动边界完整显示；
16. 底部导航图标是否仍无独立底座；
17. 选中项是否仍使用按钮级纯色半透明背景、边框和图标高亮；
18. 上下栏四边边框和圆角弧线是否完整；
19. 非页面滚动容器是否仍在边界释放滚动手势；
20. 最后一张内容卡片是否能完整滚动到导航栏上方；
21. 是否同步更新了本设计文档。
...
- 顶部状态项必须设置可访问名称，使移动端隐藏可见标签后仍保留完整语义。
...
- 移动端业务页面的断点覆盖集中在 `mobile-pages.css`，不得混入应用壳层定位规则。
...
- 桌面端双列或多列业务布局必须在 `720px` 及以下显式改为单列，不能依赖浏览器自动压缩。
- 生产页 `.production-grid` 在移动端必须使用 `grid-template-columns: minmax(0, 1fr)`。
- `.facility-list` 和 `.facility-card` 必须保持 `width: 100%` 与 `min-width: 0`，不得进入视口外的第二列。
- 移动端 `.build-card` 不得保持桌面端 sticky 定位，必须恢复为普通文档流。
- 当玩家已有设施时，设施卡片必须能够在建造卡片之后通过纵向页面滚动完整显示，不得横向隐藏。
