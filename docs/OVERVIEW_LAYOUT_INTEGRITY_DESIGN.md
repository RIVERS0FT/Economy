...
3. `390×844` 下概览没有自己的教程 DOM，同一 Outliner DOM 只显示教程并固定在状态栏下方；根级业务 Sheet 与通知覆盖教程时教程仍保持挂载，状态栏继续位于最上层；
4. `900×1000` 下概览摘要单列且无横向溢出，Outliner 默认只占 `44px` 轨道；
5. 侧栏悬浮展开覆盖概览但不改变页面和战略追踪器几何，且竖线与阴影跟随展开边缘；
6. 签到七格、短挂单和长挂单滚动语义；
...
- `src/components/shell/StrategicWorkspace.tsx`：常驻地图、镜头栏以及战略追踪器外壳挂载；
...
- `tests/browser/runtime.spec.ts` 与 `tests/browser/tutorial-right-rail.spec.ts`：概览内容、Outliner 常驻、移动覆盖层级与真实几何；
