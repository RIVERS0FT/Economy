...
- 订单簿为单列，卖盘和买盘分别只显示最优 5 笔订单，中间只使用无文字的中性分隔线。
- 订单簿内部不显示最近成交或价差指标；桌面卡片拉伸到所在网格行高度，移动端按内容自然增长。
- 市场本地成交记录不设内部固定高度、分页或垂直滚动区，面板随当前记录自然增长。
...
买入状态不显示“买入快捷数量按资金与仓库剩余空间共同计算，当前最多 …”辅助句。资金和仓库限制仍由 `maxBuy`、输入上限、按钮禁用状态、可用资金和仓库剩余摘要表达。卖出状态可以继续显示当前商品可用库存说明。
...
不得显示玩家身份。订单簿内部不得显示最近成交、价差、成交价指标或指标卡式成交线。最近成交和价差只能放在订单簿之外的行情摘要、价格曲线或全局只读状态中。

卖盘选择价格最低、时间最早的前 5 笔；买盘选择价格最高、时间最早的前 5 笔。

桌面三列和紧凑桌面两列布局中，订单簿卡片必须使用网格默认拉伸或显式 `stretch`，占满所在网格行高度。买卖盘不足 5 笔时不生成空订单占位；移动端单列布局按内容自然增长。
...
市场页本地成交记录必须遍历全部当前已加载的 `localTrades`，不得通过 `slice`、分页、折叠或内部垂直滚动限制可见记录。面板随记录数量自然增长并由页面主滚动区域承载；本地存储容量上限仍由 `docs/LOCAL_ACTIVITY_LOG_DESIGN.md` 管理。
...
15. 卖出快捷量只使用当前商品可用库存；
16. 订单簿单列且买卖盘各最多 5 笔；
17. 订单簿内部不存在最近成交、价差或 `order-book-midpoint`；
18. 桌面订单簿卡片拉伸到网格行高度且不生成虚假订单占位；
19. 市场本地成交记录遍历全部当前记录且没有内部垂直高度限制；
20. 页面不出现工厂实例、小时指标、累计产量或系统估值；
21. 挂牌工厂不参与生产，已有挂牌不阻止未挂牌数量启动；
22. 运行中撤销挂牌的数量只从下一周期加入；
23. 出售挂牌数量不得停止卖方正在运行的未挂牌集群；
24. 买入快捷数量“当前最多”辅助句和“未完成买单剩余数量”说明不会恢复。
