...
已完成工厂只允许 `running`、`stopped`、`error` 三种顶层状态。`enabled` 单独保存玩家运行意图：异常时保持为 `true`，条件恢复后从新的完整周期自动运行；玩家关闭开关或定量计划完成后为 `false`，不得自动恢复。

异常原因包括运营资金不足、共享仓库空间不足、原料不足、定量计划不兼容、没有未冻结工厂和系统维护。异常期间不扣费、不消耗原料、不产出，也不累计或追补周期。

运行中允许保存 `pendingProductionPlan`。当前周期继续使用旧计划；周期结算、待加入数量合并后再应用最后一次保存的待生效计划。若工厂处于停止或异常，新计划立即生效；开关开启时随后统一重新检查自动恢复条件。

生产开关必须使用共享 `SwitchControl`，与设置页音效开关共同使用设计系统唯一的 `.ui-switch` 视觉。不得恢复播放／暂停按钮或工厂专属开关样式。
