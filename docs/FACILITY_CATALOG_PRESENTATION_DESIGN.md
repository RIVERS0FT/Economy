...
建设下拉框、已拥有工厂选择卡和详情默认选择必须使用同一目录顺序。详情选择状态与建设下拉框状态必须独立，不能因为查看工厂详情而改变待建设工厂类型。
...
不得按 `facilityGroups` 返回顺序、中文名称、ID、运行状态、工厂数量或 `localeCompare` 重新排序，也不得把“运行中优先”“最近查看”作为默认详情规则。
