...
经济结算规则仍以产品、产业、商业建筑、拍卖和排行榜对应文档为准；普通状态轮询、修订号、分区补丁、服务器幂等事务和服务器容量以 `SERVER_ARCHITECTURE_AND_DEPLOYMENT_DESIGN.md` 为准。
...
这些截止时间必须登记在 `src/utils/authoritativeCountdowns.ts` 或其统一合同截止时间来源中。商业营业周期必须进入统一注册表，并只登记 `hasCommercialCycle(group)` 判定为真实在途的服务器截止时间；零收入恢复周期仍由 `cycleActive` 保持资格，不能按 `pendingRevenue > 0` 过滤。页面不得各自维护另一套到期刷新定时器。工厂建设即时完成，不属于倒计时系统。
...
- 不得继续显示负数时间；
- 不得为了让按钮提前恢复而在客户端删除研发、拍卖、合同、生产或商业经营状态；
...
生产产物与作业制度的单地区／批量配置、工业自动经营和商业自动经营策略都属于稳定配置意图。统一写协调层在计算指纹与实际发送之前去除这些配置请求中随权威刷新变化的 `productionSettlement`，其他业务字段、完整批量目标、玩家身份和存档世代保持不变。这样，前序配置结果未知时重新确认原目标仍使用原请求体和原幂等键，不会因轮询更新了生产提案而变成第二次业务执行。服务器仍在既有动作事务中按权威状态结算旧周期后再执行配置；独立 `/production/settle`、建设及其他非配置请求不得因此丢失自己的补算提案。客户端不能通过本规则提前确认生产、放宽服务器校验或取消实际发生的配置切换成本。

动作提交等待与确认后的状态恢复是两个独立生命周期。收到明确回执后立即结束调用方提交等待；当前已接受 authority 若已达到该回执修订号，直接复用而不再补拉，否则由 `ConfirmedActionSync` 合并到当前需要达到的最高确认修订。多个确认共享进行中的同步任务，只在读回修订仍未覆盖较新回执时串行补读；动作发生前启动的旧读取不能仅凭“已有请求”充当本次确认。同步任务存在期间暂停常规轮询，但不把同步忙碌重新当成按钮提交忙碌；必要的到期权威确认仍可在当前同步结束后继续。

操作提交状态只覆盖该逻辑写请求及其同键确认，不包含无关的只读报价、摘要、历史、通知或已确认后的补拉等待。页面不得以后台读取 busy 替代提交 busy；明确回执与最近权威状态继续按各页面规则展示，后台读取失败不撤销已确认结果。只读请求的状态须与当前查询上下文绑定，清理或迟到响应不得结束另一个上下文的请求；已有读取时限及经济写幂等确认机制保持不变。

状态恢复沿用 8 秒读取截止时间，失败、超时和 reset 均必须释放协调状态，即使底层读取忽略 Abort 也不能永久挂起。旧会话任务的迟到完成不得清除新任务或发布旧状态。动作已经确认而同步失败时保留已确认结果和最近权威状态，通过通知说明同步问题，后续读取继续恢复；不得把操作改报失败或自动重复提交配置。配置最新目标与原子批量的业务边界归生产设计，完整自动经营策略的合并与回退归仓库及商业设计。

`tests/dt/latest-configuration-queue.test.ts` 覆盖最高修订合并、旧读取、超时释放、reset 与旧回调隔离；`tests/browser/configuration-write-identity.spec.ts` 覆盖四类配置在生产提案变化后的同键确认、确认后独立新操作，以及非配置请求补算提案不被删除。
...
- 拍卖结束后继续允许出价或取消；
- 页面自行把研发、生产、商业经营、拍卖、合同或排行榜标记为已结算；
