...
跨州运输继续使用 `transportShip` 的 `cycle-start` 与 `node-service` 两类权威操作推进。客户端负责节点装卸规划，并在 `cycle-start` 提交实际出车数量；只能提交当前节点的装卸意图；服务端只结算当前到期运输段，并在每次 `node-service` 重新校验路线、周期、当前访问节点、真实车载货物、地区库存与容量。车辆抵达节点后进入 `docked`，不得因离线恢复一次跨越多个未来节点。`transportShipments` 与 `transportRoutes` 一并归入 `player.misc`，运输到站或装卸不得新增第七父分区或错误推进市场分区。
...
运输增购通过既有 `transportShip` 的 `route-expand` 处理，不另建 API 或资产账本。请求包含路线、正整数增购数量与确认时的 `expectedVehicleCount`；服务器验证归属、当前拥有数量、非预约删除、上限与资金，用共享单价计算费用，同一事务扣款并增加拥有数量。复用既有 Idempotency-Key／存档世代回执；过期表单即使使用新请求身份也不能重复增购。不得接受客户端价格、总费用、路径或运输方式覆盖。创建一律包含一个载具，不能通过创建参数免费增加数量。

启动编组时服务器按真实拥有数量校验实际出车数量，再计算总容量、现金运费和整趟取整燃料；资产校验全部通过后才原子扣费、扣油、装货和发出首段。运行记录保存数量、单体与总容量、按数量放大的费用和耗油快照，节点服务始终校验本趟快照而非路线最新运力。增购不能修改活跃记录。业务参数、价格与候选选取语义唯一归 `WAREHOUSE_EXPANSION_DESIGN.md`；一个编组维持一个运输记录和截止任务。
...
- 客户端状态版本唯一来源是 `server/shared/economy-state-version.js`；版本数值和最低兼容下限以该共享模块为准。商品燃料结算必须提高兼容下限，使旧现金燃料规划器不能继续消费新版运输投影，而通过既有入口刷新流程取得匹配客户端。运输投影继续复用 `transportRoutes` 与 `transportShipments`：路线保存不可变的路径与运输方式以及拥有载具数量，当前运输记录只携带节点循环所需的轻量当前段、当前车载摘要、每趟费用／燃料摘要和 `docked` 状态；普通玩家不存在手动 `route-dispatch`。商业目录 `commercialBuildingTypes` 与商品、工厂、研发和地区目录同为 catalog 完整快照的必需字段；否则会把同一玩家的工业建设入口保留在旧目录中而静默丢失商业建设入口。服务器响应、`src/types.ts`、浏览器合并器、README、DESIGN 和 verifier 不得维护独立版本常量。版本低于下限或高于当前值时返回明确的“客户端状态版本不兼容”，客户端只允许刷新入口 HTML，不得在旧 JavaScript 内原地重试状态请求。
...
市场储备新发布的采购合同是每日额度商品合同的固定条款系统采购方变体，必须由 `daily-supply-contracts.js` 的 `createMarketReserveDailyProcurementContract` 创建，不得再调用旧 `createMarketReserveProcurementContract` 生成有限批次。储备采购方通过 `publisherType = market_reserve + marketReserveGroupId` 直接绑定 `marketDemand.liquidity.groups` 的真实 `credits / frozenCredits`，不创建 `world.players['0']`；交付商品直接进入该组真实储备库存。当前一期合同地区固定为默认地区 `110000`，日额度只补向储备 90% 目标位且最多使用发布时可用储备资金 30% 对应的采购量，期限 1 天、无开始延迟，承接后截止时间固定为下一个北京时间自然日边界，不能跨日重置第二份额度；承接后储备与供应玩家按同一每日额度保证金／当日托管规则冻结真实资产，储备自动结算已准备额度，额度完成后立即正常结束。固定条款不得议价或续签。

正式世界处理对旧有限批次合同继续维持到期生产、自动准备、批次结算与宽限；规则切换前已经进入履约的市场储备旧合同继续按原冻结与批次快照处理到终态，旧版公开储备采购合同在下一次储备运行时直接过期且不得承接，新发布储备采购只能使用每日额度合同。每日额度合同只在实际生产输入择源、手动／自动当日托管、北京时间自然日边界和合同到期边界推进，不因“当日额度未用满”启动宽限或违约。玩家商品写动作不进入开放订单簿：服务器在同一事务内读取州×商品当日 `officialPrice` 并即时结算资金、地区库存、卖出手续费、成交记录与当日买卖量。世界级截止时间机制通过北京时间每日 00:00 唤醒完整世界处理并按前一自然日玩家↔系统买卖量失衡更新官方价；重复处理同一 `priceDateKey` 不得重复调价。`world.systemMarketAudit` 继续作为顶层 segment 随事务持久化且只用于服务器审计。服务器内部人口／储备订单撮合与玩家即时交易严格分离，普通玩家状态不得下发内部订单深度。

合同交付不得写入统一订单簿、最近真实成交价、市场价格历史、商品或工厂估值以及交易排行榜。卖方合同货款按单份合同累计成交额精确收取 1% 市场服务费，并沿既有市场服务就业规则进入人口钱包。合同状态、交付时间、宽限期与违约只能由服务器权威时间判定。旧有限批次合同（包括规则切换前已经进入履约的市场储备旧合同）的处理、动作或客户端序列化只能对旧合同视图建立一次 `contract-runtime-index.js` 派生索引；采购方下一批仓库预占、旧公开合同数量、旧参与者进行中合同数量和合同 ID 查询必须读取该索引，不得在每份旧合同容量检查中再次遍历全部合同。旧合同进入或离开 `active` 状态、完成批次、过期、取消或终止时必须同步刷新索引计数；索引不进入世界 JSON、客户端分区、分区哈希或 SQLite，事务回滚后直接丢弃。每日额度商品合同由 `daily-supply-contracts.js` 的地区＋商品＋参与方条件直接派生当日额度和托管状态，不得伪造“下一批仓库预占”，也不得为了兼容旧索引重复建立第二个全合同索引。

合同审计必须同时区分新每日额度事件与旧兼容事件。每日额度合同的追加式审计覆盖发布、承接、议价、自动准备／补款设置变化、手动准备／补款、每次真实部分交付、当前日结束申请、有限合同到期、立即违约终止和保证金赔付；北京时间跨日的未用额度释放与重置属于权威状态推进，当前不伪造独立审计事件。交付事件必须记录真实 `lastDeliveryQuantity / lastDeliveryGross / lastDeliveryFee`，不得把每日上限伪装成实际交付量。旧有限批次合同继续保留原发布、批次准备／补款、宽限、逐批成功交付、续签、违约确认与受偿方领取事件。每个事件保存稳定事件类型、执行主体、触发类型、服务器事务时间、原因代码、前后紧凑快照和资产转移；玩家查询不得返回请求幂等键、来源哈希、世界修订号或其他玩家的非合同资产。旧世界既有合同只允许按已有审计完整度读取，不根据缺失字段伪造过程。 玩家冻结借贷的新违约清算必须使用单一 `loan_bank_guarantee_settled` 终态事件记录未履行额度、银行代付到账、银行存款代收、可用资金代收、冻结工厂代收、工厂处置找零、风险准备金吸收和担保发行；不得再把新违约贷款审计为冻结工厂转给出借方。历史既有 `loan_defaulted / loan_default_claimed` 只作为旧审计兼容读取，不再由新违约流程产生。
...
玩家贷款承接时从出借方可用资金原子转移本金到借款方，并锁定借款方冻结工厂；银行不得替代合同成立时的本金出资。冻结工厂继续生产，但不得出售、拍卖、银行冻结、重复借贷冻结或出租。到期优先按自动还款设置从借款方可用资金结清本金和固定利息；不足进入通用宽限。宽限结束再次尝试正常还款后仍不足时，服务器必须在一个幂等事务完成银行担保清算：固定当时未履行额度与当前审慎价格 80% 的处置单价，银行向出借方代支付应收净额并按既有规则结算贷款利息服务费，同时依次从借款方银行存款、可用资金、按处置单价足以覆盖剩余欠款的最少冻结工厂执行银行代收。取得的工厂进入 `world.bank.facilityReserves`，不得转给出借方；整座工厂折抵超过剩余欠款的价值返还借款方银行存款，未处置冻结立即解除。银行以取得冻结工厂的实际折抵价值发行对应清算信用；若数据异常、参与者缺失或资产不足仍形成缺口，则先由 `riskReserveCredits` 吸收，再把剩余缺口显式记入 `bankGuaranteeIssuedCredits`。清算完成后合同直接写入 `breachedAt + borrower_default + terminated`，未偿本金与利息归零，不再存在出借方“解除合同并处置冻结”动作；旧状态中已经处于 `breachedAt + borrower_default + active` 的贷款在下一次权威处理时按同一规则惰性清算。重启、迟到轮询、重复截止时间、幂等重放或旧状态迁移不得重复代付、重复代收、重复扣厂或按新价格二次清算。
...
公开经济事件日历由服务器固定模板和时间槽确定，不持久化第二套可编辑事件数据。状态快照在现有六分区中的 `market` 分区返回 `economicCalendar`，客户端由玩家外壳统一消费；地图、追踪器和详情入口归 `PAGE_CONTENT_AND_NAVIGATION_DESIGN.md`，不进入概览或市场页面正文；返回内容不包含逐请求变化的 `visibleUntil`。世界截止时间规划器只纳入事件开始和结束边界，不再为了事件进入滚动七天可见窗口唤醒世界。需求运行时仅在既有预算分配阶段应用事件权重，保持总预算、真实人口钱包、市场储备和货币发行守恒。
...
商品投资账户、已投入现金生产快照、历史正式价及库存退役计划属于服务器权威数据，业务含义分别归商品市场、产业和仓库退役设计。它们不是客户端可提交的完整资产对象；客户端不能决定成本、利润、工资、日期、指数权重或迁移版本。

现金经济内核先完整验证并准备金额、数量、市场记账和游标，再交由原权威经济事务提交。投资本金与损益入账、销售费用进入人口就业、生产工资分配、服务器审计、SQLite 保存、权威修订与幂等确认必须在同一个正式事务中完成。领域函数返回的就业分配结果不能在异步事务或客户端中另行补记。失败回滚必须恢复资金及所有受影响段，不能只回滚前端可见余额。

迁移只允许服务器内部持有的计划对象执行，并重新验证世界摘要；计划不得持久化为可绕过校验的客户端凭证。迁移是冷路径，可以在隔离草稿上核对整个世界；正式逐周期写入仍须沿用限定玩家、行情键和资金段的 Copy-on-Write，不能以迁移实现为由在每个操作复制完整世界。新的保存字段必须纳入完整存储、作用域、状态分区和经济不变量；已有固定六父分区不因此增加。

正式入口启用必须同时具备：生产及商业的原料和收入结算接入、独立投资账户的动作与到期推进、估值／授信／周资金结算／删除存档的资产口径、运输和实物业务退役、旧资产迁移及前端入口切换。不得只写入现金经济标记、只开放投资接口或只隐藏仓库后继续执行库存型自动出售。任一迁移阻塞或联动门禁未完成时，保留旧状态，不执行生产数据切换；完成这些门禁之前，不允许将独立内核测试通过等同于新玩法上线验收。

报价、估值、页面打开与普通状态读取不得补建持仓、自动平仓或进行库存迁移。正式价格初始化与归档是明确的权威写路径，必须具有有界扫描与重复执行保护；归档保留期限及未结算引用由商品市场设计定义，服务器负责与日界调价及截止时间调度按确定顺序提交。

### 投资动作、估值与追加式审计

`POST /api/game/investments/commodities` 是独立注册的 `tradeCommodityInvestment` 动作，复用认证、orders 限流、存档世代、服务器动作幂等、限定玩家 Copy-on-Write 和精简确认。载荷只接受商品、合约身份、报价日期、方向及数量；不读取客户端完整账户或成本，旧商品买卖载荷不能解释为期货。未通过现金经济启用门禁时服务器明确拒绝该动作，普通客户端保留旧入口，不因账户读取写入启用标记。

客户端投资请求不携带会改变幂等身份的生产补算提案。账号及存档世代内的未确认投资只能先确认原请求；日期、商品或数量改变后，先恢复原请求结果，再要求玩家显式重提新交易，不能像启停目标一样自动执行第二笔金融交易。重复点击合并正在执行的同意图请求，切页与重载继续保留原幂等身份。超过服务器幂等保留期的未知交易必须保持阻断，不能自动换键重放。

公开指数报价归入既有 `market.quotes`，私人持仓、权益、费用和最近流水归入 `player.bank`；完整审计和全服投资总账不下发玩家状态。协议版本 45 明确支持未知资产估值，客户端不得将 `null` 持仓权益或净资产格式化为零。已知的原料库存等兼容字段继续由各自领域管理，不把它们复制为投资持仓。

`economy_commodity_investment_audit` 保存投资追加式审计，唯一键由玩家、存档世代与单调流水身份组成；真实完成时间、处理时间和前后权威修订分别保存。审计、持仓、总账、就业费用、世界修订及动作回执在同一 SQLite 事务提交；审计插入失败或重复事件必须回滚全部经济写入，不能忽略唯一性冲突。玩家最近 100 条展示历史可以裁剪，完整审计不随裁剪或删档删除。

到期调度复用已有世界截止时间计划，按到期合约取最早时刻，不新增逐秒轮询；已过期但缺历史价的持仓使用有界重试时间，不能零延迟空转。日界处理先提交地区正式价及归档，再结算当期到期持仓，随后执行银行、周资金结算与相应排行榜处理。强制世界推进与普通计划执行必须保持同一经济顺序。

待估值周账单仅保存原评估基线，不是第二份可用资金或持仓；恢复时校验合约身份、去重、原到期日和累计费用，使用原日历史价计算一次。该基线属于玩家既有周结算状态并随同一事务持久化，不能在读请求中生成债务或在重新登录时覆盖成新余额。投资、借贷和删档的玩家资格语义由产品设计拥有。
