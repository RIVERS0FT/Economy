...
- Resend 配置从 `/etc/riversoft-economy-api.env` 读取，至少包含 `RESEND_API_KEY` 和 `EMAIL_FROM`；配置文件不得提交到仓库或打印到日志。
- 邮件密钥只保存在服务器。GitHub Actions 不保存、不上传也不改写 `RESEND_API_KEY` 或 `EMAIL_FROM`；邮件配置工作流只通过 SSH 验证服务器已有配置。
- `.github/workflows/configure-registration-email.yml` 在主部署成功后运行，确认环境文件包含两个正式变量，随后重启 `riversoft-economy-api.service`。
- 配置工作流必须从运行进程的 `/proc/<pid>/environ` 验证 `RESEND_API_KEY` 与 `EMAIL_FROM` 均为非空，并写回 `deploy/economy-email` 提交状态。不得只检查文件存在或空注册请求返回 400。
...
`npm run build` 必须执行设计与架构验证、Nginx 测试、服务器语法和测试、TypeScript 与 Vite 构建。主部署后验证 API 健康、静态网页、账号代理、注册代理、未登录 401、systemd 用户／端口／数据库、注册秘密和无重复路由；邮件配置工作流另行验证服务器环境文件、运行进程实际环境和服务重启，并以 `deploy/economy-email` 独立状态防止主部署误报验证码可用。
