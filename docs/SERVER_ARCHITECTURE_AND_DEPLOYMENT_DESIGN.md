...
旧 C2 作业制度 `rapid`／`economical`／`high-yield` 仅在服务器内部作为迁移别名识别，迁移到同一基础产物路线的标准制度后不再公开或接受主动选择。持久化世界版本固定为 31；市场需求模型 19 对新增燃料和工业化学品建立直接需求，并在模型 18 升级时释放并重建人口消费与市场储备系统订单，不改写玩家真实资产和成交历史。
