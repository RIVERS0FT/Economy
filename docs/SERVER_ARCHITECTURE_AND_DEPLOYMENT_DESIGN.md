...
- 生产验证码投递必须同时配置 `RESEND_API_KEY` 与 `RESEND_FROM_EMAIL`。缺少任一项时接口返回 `424` 和“邮箱验证码服务未配置，请联系管理员”，不得把配置缺失伪装成整个游戏服务器故障；Resend 超时、拒绝或无效响应继续返回通用 `503`，不得向客户端泄露供应商响应正文。
...
- Resend 配置从 `/etc/riversoft-economy-api.env` 读取，至少包含 `RESEND_API_KEY` 和 `RESEND_FROM_EMAIL`；配置文件不得提交到仓库或打印到日志。
- `.github/workflows/configure-registration-email.yml` 在主部署成功后运行。若仓库 Secrets 同时存在 `RESEND_API_KEY` 与 `RESEND_FROM_EMAIL`，工作流以 `0600 root:root` 安全写入环境文件；若两个 Secrets 都未提供，只允许复用服务器已有的完整环境文件；只配置其中一项必须失败。
- 配置工作流重启 `riversoft-economy-api.service` 后必须从运行进程的 `/proc/<pid>/environ` 验证两个变量均为非空，并写回 `deploy/economy-email` 提交状态。不得只检查文件存在或空注册请求返回 400。
...
`npm run build` 必须执行设计与架构验证、Nginx 测试、服务器语法和测试、TypeScript 与 Vite 构建。主部署后验证 API 健康、静态网页、账号代理、注册代理、未登录 401、systemd 用户／端口／数据库、注册秘密和无重复路由；邮件配置工作流另行验证 Resend 环境文件、运行进程实际环境和服务重启，并以 `deploy/economy-email` 独立状态防止主部署误报验证码可用。
...
- 在缺少 `RESEND_API_KEY` 或 `RESEND_FROM_EMAIL` 时仍把部署标记为验证码可用，或把明确的邮件配置缺失重新改写成整个游戏服务器不可用。
