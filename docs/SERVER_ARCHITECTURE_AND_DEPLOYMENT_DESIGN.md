...
世界 20 迁移初始化周结算状态和银行版本 3，不追溯规则上线前利息或扣除，并把上线所在周标记为不完整周；下一个完整周开始正式关闭活跃周账单。客户端状态版本 23 增加周结算摘要。重复加载、登录重试、服务重启与跨周追赶不得重复建账或重复扣款。
