...
世界 32 是当前持久化边界，当前客户端状态版本为 39，世界存储 schema 为 V2。`world.bank` 等世界级银行状态写入对应顶层 segment，`player.bankAccount`、贷款抵押明细和玩家统计随对应玩家行保存；它们仍与玩家资金、工厂、订单、拍卖和合同共享同一全局修订号、SQLite 事务和回滚边界，不允许形成可独立提交的第二套余额权威。银行最近记录只保留每名玩家最近 100 条，普通客户端序列化最近 50 条；利息微单位余数和资金池微单位仍是服务器内部整数，不得暴露为可直接使用的普通货币。

世界 30 在原世界内增加州级经济维度，不创建 48 份世界。商品库存、商品行情与工厂行情使用 `provinceId:assetId` 作为顶层记录键；`provinceId` 只是保留的内部兼容字段名。工厂集群和订单保存显式 `provinceId`；拍卖每个 `items[]` 项保存显式 `provinceId`；银行贷款每项抵押以及玩家借贷、工厂租赁合同保存工厂标的 `provinceId`。订单簿运行时索引、Copy-on-Write 市场键、抵押／租赁锁定、排行榜估值和资产统计必须把地区纳入身份，任何跨地区订单不得成为候选，同类型工厂不得跨地区被锁定或处置。六分区继续保持六个父分区：目录分区返回地区目录，玩家分区返回 `provinceInventories`、`provinceFacilityGroups` 与地区资产摘要，市场分区返回不含 `priceHistory` 的 `provinceMarkets`／`provinceFacilityMarkets` 摘要与仅本人订单；默认 `inventories`、`facilityGroups`、`markets` 和 `facilityMarkets` 只作为加利福尼亚州兼容投影。
...
`GET /api/game/market-detail?provinceId=&assetKind=&assetId=&revision=` 是商品与工厂当前资产完整行情和匿名五档的唯一按需读取。服务器校验地区和正式目录身份，返回完整有界公开行情、买卖各最多五个聚合价格档、详情修订与 `serverNow`；修订命中时省略详情正文并返回 `unchanged: true`。浏览器最多缓存 32 个资产详情，切换资产时中止旧请求。`GET /api/game/facility-build-quote?provinceId=&facilityTypeId=&quantity=` 是建筑表单真实全量卖盘的只读预扫，返回逐材料价格上限、默认挂单价、预计总额、缺口和自交叉提示；两类 GET 必须直接读取 committed world，库存缺失按零值读取且不得触发迁移、补键或兼容别名写入，从而复用订单簿运行时索引并保持投影无副作用；同时不得写 SQLite、推进世界修订或进入六分区轮询。
...
普通玩家权威动作的持久化幂等确认仍固定为 `{ result: { ok, message }, revision }`；动作事务和 `economy_idempotency.response_json` 只生成并保存这份精简确认，不得保存订单 ID、兑换数量、结算金额、完整状态、分区补丁或其他动作内部字段。HTTP 传输层在事务提交后必须从当前 committed world 为当前玩家生成一次权威状态交付，并把精简动作结果与 `commandRevision`、顶层状态 `revision`、`serverNow`、`partitionRevisions`、`sliceRevisions`、`unchanged` 和按请求头 `X-Economy-State-Revisions` 过滤后的 `patches` 组合为同一次响应。`commandRevision` 表示该命令实际提交时的世界修订号；响应的顶层 `revision` 表示交付快照修订号且不得低于 `commandRevision`。幂等重试可以继续复用原命令结果，但必须附加重试时可取得的当前权威状态交付，不得把历史状态快照写进幂等表。

客户端收到动作响应时必须先把其中的权威状态交付送入与 `GET state` 共用的状态交付缓存，再结束该动作的本地 pending；正常成功路径不得为了取得同一动作结果再追加一次 `GET state`。若动作响应中的权威增量因客户端本地完整性错误未能接受，补拉失败不得把已经提交成功的动作改写为失败；客户端保留已确认结果并由非阻塞 `GET state` 或后续普通轮询恢复。服务器确认前仍不得把资金、库存、州权限、订单、生产结果等权威经济状态写进 authority；动作响应与 `GET state` 使用同一 revision、saveEpoch、catalog 完整性与分区整块替换门禁。

具有确定目标值且失败可恢复的直接控制允许维护独立于 authority 的客户端 Intent Overlay。当前首个实例是工厂启停：点击 Switch 的同一交互周期必须立即显示玩家最新目标值，同一 `provinceId + facilityTypeId` 的快速连续目标按顺序发送且只允许一个请求在途；较早请求完成不得覆盖较新的目标。Intent 只允许影响控件展示，不得写入 `EconomyState`、参与资产计算、生产结算、资格判断或其他权威推导。服务器拒绝当前最新目标时立即删除 Intent 并恢复最近 authority；服务器成功且动作响应已被 authority 接受时立即删除 Intent；若成功响应已提交但本地增量验收失败，则保留已确认 Intent，直到后续权威状态与目标一致后清除。直接控制的成功以控件状态本身反馈，失败通过错误反馈说明并回退。
...
正式世界处理对旧有限批次合同继续维持到期生产、自动准备、批次结算与宽限；每日额度合同只在实际生产输入择源、手动／自动当日托管、北京时间自然日边界和合同到期边界推进，不因“当日额度未用满”启动宽限或违约。商品订单写动作在真实订单簿撮合完成后执行官方系统价实时清算；世界推进在市场需求周期结算后推进每州×商品的官方系统价价格周期，并把 `world.systemMarketAudit` 作为顶层 segment 随事务持久化（该 segment 只用于服务器审计，不进入六分区或普通玩家状态）。跨州运输把 `world.transportShipments` 作为顶层 segment 随事务持久化，并在世界截止时间规划器注册运输到达边界；多站点运输继续按 `stopPlan` 注册“下一未交付站”的最早截止时间，shipment 同时固化多商品 `manifest` 与逐段权威时间／载荷 `legPlan`。到达某站时只把 `manifest` 中目的地为该站的源州 `inTransit` 转入该站 `available`，整链完成后才把 shipment 置为 `arrived`。玩家运输路线直接保存在对应玩家分段状态的 `transportRoutes` 中，每人最多 50 条、站点序列无上限；运输路线不保存商品、数量或自动发运开关，只保存 ID、名称、路径、行程、运输方式和时间戳。路线创建、地图编辑保存、重命名和删除继续复用 `transportShip` 权威动作，普通玩家不存在 `route-dispatch` 手动发运操作。有运输在途时禁止删除路线，路线编辑不得改写已经出发 shipment 的 `manifest`、`stopPlan` 或 `legPlan`。服务器只在既有正常世界推进末尾检查等待路线：同一路线没有在途运输、玩家全局在途未达上限，并且按当前库存、州级参考价、运输容量和资金能够形成整趟正预期净价差 `manifest` 时立即自动发运；条件不足只保持等待，不产生失败写入。每条路线同时最多一笔在途运输，整链完成后可在同一次正常世界推进末尾重新检查下一趟；不得新增运输专用轮询器、离线定时器或后台扫描器。旧有限批次商品合同继续在单一事务中按原批次规则检查冻结商品、冻结货款和兼容仓库条件，并保留“宽限结束只确认违约 → 受偿方主动解除”的两阶段状态机。每日额度商品合同没有整批强制交付：每次生产输入或显式使用只结算 `min(请求数量, 当日剩余额度, 已托管货款可支付数量, 已预留商品数量)`，允许同一自然日多次部分使用；额度、货款或预留不足时只停止本次合同消费并由生产流程继续尝试本地仓库／统一市场，不因为未消费的额度进入宽限。旧有限批次商品合同的 **宽限结束只确认违约**：服务器写入 `breachedAt` 与责任代码，清除下一到期时间并进入“已违约待解除”，释放与赔偿无关的当前货款、商品和无责方保证金，只继续冻结责任方保证金。已违约待解除合同不得继续自动准备、自动补款、手动补救、续签或恢复履约；合同运行时索引不得继续为其创建下一批仓库预占或截止时间。受偿玩家随后通过既有终止动作主动解除并原子领取违约金，责任方提交同一动作必须被拒绝；双方违约允许任一参与方关闭且不发生赔付。市场储备作为受偿主体时没有玩家操作入口，因此仍由系统自动完成处置；市场储备自身违约而玩家为受偿方时必须等待玩家主动领取。
...
资源不足时宁可拒绝写操作，也不能产生负库存、重复发放、重复扣款、重复邀请奖励、重复合同交付或超额成交。资产包拍卖必须先预检全部 `items[]` 再冻结，并在结算前再次验证全部归属、冻结资金、商品仓库和工厂数量；任一项目失败时回滚整包。新每日额度商品合同必须先预检双方、地区权限、日额度保证金与当前日自动托管能力再进入进行中状态；实际交付允许在当日额度内分多次部分结算。旧有限批次商品合同继续按其首批货款、保证金和原批次原子规则兼容。资产毛值只从可用／冻结资金、银行存款、库存和工厂归属等权威余额派生，净资产必须扣除贷款本金与利息；拍卖、合同和银行抵押记录不得重复计价。
...
当前客户端状态版本为 39，本地匿名成交存储为 v7，世界状态版本为 32，市场需求模型版本为 20；人口迁移、手续费迁移、储备迁移、资产拍卖删除迁移、产业目录迁移与州级经济目录替换都不得重置商品／工厂资产、玩家订单或既有订单簿成交，储备种子只允许初始化一次。
...
- 服务器 `industry-catalog.js` 是生产方式数值的唯一正式来源，并通过 `production-methods.js` 把每个基础配方编译为该工厂四个具名作业制度配方变体。
- 生产方式不新增世界状态字段、不新增动作路由，也不改变工厂资产结构；现有 `setFacilityRecipe` 动作接收目标变体 ID，继续复用 `activeRecipeId`／`pendingRecipeId` 和立即原子切换事务。
- 每座工厂的第一种制度是 `defaultMethodId` 并保留基础配方原 ID；普通玩家状态中的 `facilityTypes[].recipes` 只公开该默认路线，客户端从 `productionMethodGroups` 元数据读取完整变体。制度稳定 ID 和 `iconId` 属于客户端状态版本 38 的必需契约。
...
服务器生产目录继续把基础配方编译为生产方式配方变体；26 类工厂的当前制度全部由工厂专属蓝图生成，不得保留通用制度目录或无蓝图回退。`setFacilityRecipe` 必须先校验工厂准入科技，再校验目标制度的 `requiredTechnologyIds`；客户端禁用状态不得替代服务器校验。普通玩家状态中的 `facilityTypes[].recipes` 只公开各工厂 `defaultMethodId` 路线，完整当前制度、稳定 ID 与 `iconId` 通过 `productionMethodGroups` 下发。

旧 `standard`／`rapid`／`economical`／`high-yield`／`assisted`／`intensive`／`mechanized` 仅在 `legacy-production-methods.js` 识别；迁移按工厂和位置映射到同一基础产物路线的同参数新制度，保留运行周期起点、满员率与批次余数，旧 ID 不再公开或接受主动选择。不同工厂允许复用同一新制度 ID，但共享定义的名称、图标和研发要求必须一致。

市场需求模型 19 为燃料与化学品建立直接需求；从旧模型升级时释放并重建人口消费与市场储备系统订单，不改写玩家真实资产和成交历史。
