...
`GET /api/game/state` 继续返回单调递增的世界 `revision`，并在网络层把客户端状态固定拆为 `catalog`、`player`、`market`、`auction`、`leaderboard` 五个分区。首次请求不带已知值，返回五个完整分区、各分区内容哈希和全局修订号；后续使用 `?revision=N&catalog=...&player=...&market=...&auction=...&leaderboard=...`。全局修订号相同时只返回 `{ revision, unchanged: true }`；全局修订号变化时只返回内容哈希变化的分区。若其他玩家推进了世界修订号、但当前查看者的五个分区均未变化，则返回新的全局修订号、完整分区修订记录、空 `patches` 和 `unchanged: true`，不得重复上传完整状态。
...
分区修订是 SHA-256 状态内容哈希的短令牌，只用于判断分区正文是否需要发送；迟到响应排序仍只使用单调递增的全局 `revision`。普通玩家权威动作响应固定为 `{ result: { ok, message }, revision }`：`result` 只能包含布尔 `ok` 和用户可见 `message`，不得携带订单 ID、兑换数量、结算金额或其他动作内部字段，也不得附带 `state`、`patches`、`partitionRevisions` 或 `unchanged`。动作事务和 `economy_idempotency.response_json` 只生成并保存这份精简确认；读取部署前 24 小时内仍存在的旧完整幂等响应时，必须先投影为相同精简确认再返回。
...
| GET | `/api/game/state` | 获取五分区初始状态、增量补丁或修订号轻量确认 |
...
正式客户端默认每 5 秒轮询一次修订号，可选 3／5／10 秒，不得恢复每 1 秒完整状态轮询。客户端根游戏模型不得维护每秒 `now` 状态；倒计时与进度只在概览、生产和拍卖等实际需要时间变化的局部页面维护，市场订单簿、资产页、导航和静态卡片不得被全局秒级时钟重渲染。局部时钟必须从最近一次权威快照的 `lastProcessedAt` 起步，并只叠加该快照在当前浏览器接收后的经过时长；不得直接以客户端墙上时间替代服务器时间。管理员入口、游戏入口和九个游戏页面必须使用动态 `import()` 按需拆包。只有 `GET state` 的分区交付响应可以更新 `EconomyState` 和客户端已接受修订号；动作确认不直接更新界面状态，其 `revision` 只用于校验随后补拉的状态不得落后。低修订号或缺少修订号的迟到状态响应不得覆盖新状态。
...
- 空闲读取或管理员摘要无变化时写库、同修订号轮询进入 SQLite 事务、每次轮询解析／序列化完整世界、修订变化时无条件上传完整状态、把变化分区浅合并进旧完整状态、服务器省略字段后客户端仍保留旧值、动作响应附带完整状态／分区补丁／分区修订／`unchanged`／内部结算字段、动作幂等缓存保存完整客户端状态、补拉前把动作确认修订号写成已接受修订号、排行榜原型钩子重复处理、每秒完整状态轮询或迟到响应覆盖新状态；
