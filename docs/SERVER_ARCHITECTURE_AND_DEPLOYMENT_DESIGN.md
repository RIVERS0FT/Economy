...
- 客户端状态版本唯一来源是 `server/shared/economy-state-version.js`；当前版本与最低兼容版本均为 39，当前客户端状态版本为 39，当前客户端只接受版本 39。客户端状态版本 39 将运输路线重构为自动物流通道：路线只保存路径与运输方式，运输实例固化多商品 `manifest` 与逐段 `legPlan`，普通玩家不存在手动 `route-dispatch`。服务器响应、`src/types.ts`、浏览器合并器、README、DESIGN 和 verifier 不得维护独立常量。版本低于下限或高于当前值时返回明确的“客户端状态版本不兼容”，客户端只允许刷新入口 HTML，不得在旧 JavaScript 内原地重试状态请求。
...
- 六分区状态交付、父分区 revision、可选 `sliceRevisions` 和按需详情以 `state-partitions.js`、`state-delivery.js` 与 `market-detail` 实现为权威。运输路线归入 `player.misc`，真实运输记录归入 `market.misc`，不得新增第七父分区。
