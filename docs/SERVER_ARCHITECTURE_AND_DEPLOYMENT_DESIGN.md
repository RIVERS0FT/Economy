...
- `game.riversoft.top` 公网链路暂时不可用期间，正式部署的外部公网验收来源固定使用 `http://123.60.108.5`，并验证 `/economy/`、`/economy-api/me`、`/economy-api/game/state`、`/economy-api/login` 与 `/economy-api/registration/email-code`；服务器本机仍必须以 `Host: game.riversoft.top` 验证正式域名的 Nginx 路由，禁止因为临时 IP 入口删除、改名或弱化正式域名配置。只有公网域名恢复且上述网页与 API 已独立通过 `https://game.riversoft.top` 验证后，才允许把 Deploy 外部验收来源切回正式域名。
