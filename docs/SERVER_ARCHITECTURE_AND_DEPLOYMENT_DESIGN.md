...
- `provinces.js`：34 个省级地区共享目录、地区规范化、`provinceId:assetId` 权威键、世界 30 迁移与默认地区运行时兼容别名；
...
世界 30 是当前持久化边界，当前客户端状态版本为 34，世界存储 schema 为 V2。`world.bank` 等世界级银行状态写入对应顶层 segment，`player.bankAccount`、贷款抵押明细和玩家统计随对应玩家行保存；它们仍与玩家资金、工厂、订单、拍卖和合同共享同一全局修订号、SQLite 事务和回滚边界，不允许形成可独立提交的第二套余额权威。银行最近记录只保留每名玩家最近 100 条，普通客户端序列化最近 50 条；利息微单位余数和资金池微单位仍是服务器内部整数，不得暴露为可直接使用的普通货币。

世界 30 在原世界内增加省级经济维度，不创建 34 份世界。商品库存、商品行情与工厂行情使用 `provinceId:assetId` 作为顶层记录键；工厂集群和订单保存显式 `provinceId`；拍卖每个 `items[]` 项保存显式 `provinceId`；银行贷款每项抵押以及玩家借贷、工厂租赁合同保存工厂标的 `provinceId`。订单簿运行时索引、Copy-on-Write 市场键、抵押／租赁锁定、排行榜估值和资产统计必须把地区纳入身份，任何跨地区订单不得成为候选，同类型工厂不得跨地区被锁定或处置。六分区继续保持六个父分区：目录分区返回地区目录，玩家分区返回 `provinceInventories`、`provinceFacilityGroups` 与地区资产摘要，市场分区返回 `provinceMarkets` 与 `provinceFacilityMarkets`；默认 `inventories`、`facilityGroups`、`markets` 和 `facilityMarkets` 只作为北京兼容投影。

由世界 29 及更早升级时，缺少地区的库存、行情、工厂集群、订单和拍卖项目归入北京 `110000`。迁移必须保持玩家订单及其数组顺序、资金、可用／冻结数量、工厂数、行情历史和拍卖托管，不得复制历史或取消订单。服务器运行时为仍使用旧默认地区入口的内部代码提供不可枚举访问别名；别名不得进入 JSON、SQLite、状态哈希、资产总计或分区 Dirty Set。`scripts/verify-provincial-economy.mjs` 与 `server/test/provinces.test.js` 固定验证 34 个唯一 ID、迁移守恒、跨地区不撮合、同地区 FIFO、本地建造生产与工厂转让。
...
当前客户端状态版本为 34，本地匿名成交存储为 v7，世界状态版本为 30，市场需求模型版本为 19；人口迁移、手续费迁移、储备迁移、资产拍卖删除迁移、产业目录迁移与省级经济迁移都不得重置商品／工厂资产、玩家订单或既有订单簿成交，储备种子只允许初始化一次。
