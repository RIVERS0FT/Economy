...
- `main` 分支由 `.github/workflows/deploy.yml` 对实际待部署提交重新执行 `npm ci` 与 `npm run build`，构建成功后再上传、安装并执行线上验证。
...
- 恢复与 CI 或 Deploy 重复执行完整构建的独立 `web-build.yml`、取消同一 PR 的旧运行，或在成功时长期上传浏览器测试 artifact；
- 删除游戏 JSON gzip；
