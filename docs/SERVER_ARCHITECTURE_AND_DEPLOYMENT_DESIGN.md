...
投资写动作使用独立的认证路由 `/api/game/investments/trades` 与 `tradeCommodityInvestment` 注册项；复用写队列、玩家及存档身份、限流、Copy-on-Write、SQLite 事务、幂等键和精简确认。旧商品交易路径不重解释为投资动作。投资持仓及最近记录归 `player.bank`，共享商品指数报价归 `market.quotes`，世界经营模式归 `catalog`；不新增第七父分区。

投资完整流水以用户、存档世代和单调序号作为 SQLite `economy_investment_ledger` 唯一键，与世界资产及审计一起提交。最近记录裁剪不删除完整账本；同身份重复写入必须内容一致。系统审计分别记录本金托管／释放、损益发行／回收和费用，账户本金不复制为冻结现金。违约追偿只能由银行内部调用最少整单位平仓，余款保留，客户端不能指定追偿成交类型。未平仓投资及未完成已付经营周期阻止删除存档，直到玩家关闭后续经营并完成合法结算。

日界正式价格先归档再更新，完整同日批量仅复制一次日期索引；之后按既有截止时间调度处理现金生产、期货到期、银行及周资金结算。现金生产和期货的缺失历史价保留其原始资产，并以有界截止时间重试，不用当前价格回填。只读资产总值、银行授信与周扣除预测允许返回明确的 `null` 及估值状态，不能把缺失投资权益当作零资产；正式授信和结算仍要求完整报价。财富排行不将估值未知玩家排为零资产。

现金生产客户端提议不能替代已付周期快照，服务器仅按自己的快照和时间完成一次结算。归一化必须保留关闭意图、配方切换、扩建或零持有数量后的已付周期；正常投影不下发内部工资系数或完成游标。只读状态、切页和投资页面打开不触发迁移。
