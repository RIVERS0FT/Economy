...
世界 32 是当前持久化边界，当前客户端状态版本为 36，世界存储 schema 为 V2。`world.bank` 等世界级银行状态写入对应顶层 segment，`player.bankAccount`、贷款抵押明细和玩家统计随对应玩家行保存；它们仍与玩家资金、工厂、订单、拍卖和合同共享同一全局修订号、SQLite 事务和回滚边界，不允许形成可独立提交的第二套余额权威。银行最近记录只保留每名玩家最近 100 条，普通客户端序列化最近 50 条；利息微单位余数和资金池微单位仍是服务器内部整数，不得暴露为可直接使用的普通货币。

世界 30 在原世界内增加州级经济维度，不创建 48 份世界。商品库存、商品行情与工厂行情使用 `provinceId:assetId` 作为顶层记录键；`provinceId` 只是保留的内部兼容字段名。工厂集群和订单保存显式 `provinceId`；拍卖每个 `items[]` 项保存显式 `provinceId`；银行贷款每项抵押以及玩家借贷、工厂租赁合同保存工厂标的 `provinceId`。订单簿运行时索引、Copy-on-Write 市场键、抵押／租赁锁定、排行榜估值和资产统计必须把地区纳入身份，任何跨地区订单不得成为候选，同类型工厂不得跨地区被锁定或处置。六分区继续保持六个父分区：目录分区返回地区目录，玩家分区返回 `provinceInventories`、`provinceFacilityGroups` 与地区资产摘要，市场分区返回 `provinceMarkets` 与 `provinceFacilityMarkets`；默认 `inventories`、`facilityGroups`、`markets` 和 `facilityMarkets` 只作为加利福尼亚州兼容投影。

由世界 29 及更早升级时，缺少地区的库存、行情、工厂集群、订单和拍卖项目归入默认地区加利福尼亚州 `110000`。中国地图时期世界 30 已使用的 34 个地区 ID 保持原值并一一对应 34 个州，其余 14 个州使用新增 ID，所以升级美国本土地图不得改键、移动、合并或复制既有地区资产。迁移必须保持玩家订单及其数组顺序、资金、可用／冻结数量、工厂数、行情历史和拍卖托管，不得复制历史或取消订单。服务器运行时为仍使用旧默认地区入口的内部代码提供不可枚举访问别名；别名不得进入 JSON、SQLite、状态哈希、资产总计或分区 Dirty Set。`scripts/verify-provincial-economy.mjs` 与 `server/test/provinces.test.js` 固定验证 48 个唯一 ID、连续州目录、旧 ID 稳定、迁移守恒、跨地区不撮合、同地区 FIFO、本地建造生产与工厂转让。
...
| POST | `/api/game/bank/loans` | 按 `provinceId + facilityTypeId + quantity` 工厂抵押明细与服务器额度评估发放唯一进行中贷款 |
...
工厂抵押只限制转让。服务器计算工厂生产可用数量时只扣同地区订单簿和拍卖冻结，计算订单簿卖单、拍卖和新增抵押可转让数量时再扣同一 `provinceId + facilityTypeId` 的当前贷款抵押。贷款评估使用抵押地区的 `min(systemValue, lastTradePrice)`，当地无真实成交时使用 `systemValue`；客户端预览不得覆盖服务器重新计算。放款同时增加玩家可用资金和同额本金负债；还本从玩家资金扣除并减少本金负债，二者不进入人口就业收入。
...
正式服务的每个 60 秒请求指标窗口最多保留 256 个方法／归一化路由键，并预留统一溢出项 `OTHER /api/other`；超过上限的未知或高基数路由只能累计到该项，不得继续扩张内存 Map、排序集合或日志路由维度。窗口结束后必须同时清空路由聚合与溢出请求计数。每个路由必须同时汇总请求总耗时的平均值、p50、p95、p99、最大值和应用层 JSON 响应字节数。响应字节以 `sendJson` 写入请求上下文的 `responseJsonBytes` 为优先权威来源，`Content-Length` 只允许作为无应用层字节指标时的回退；响应结束后头部不可读不得把已经记录的非零 JSON 响应误记为 `0 B`。同一请求上下文必须记录事务等待、世界草稿解析／复制、玩家动作前快照、世界推进、经济不变量检查、资金规范化、世界等值比较、状态投影、合同投影、分区构造、分区哈希、世界序列化、世界 SQLite 更新、响应序列化和 SQLite 提交等阶段耗时，其中正式名称至少包括 `playerSnapshotMs`、`economicInvariantMs`、`worldEqualityMs`、`serializeWorldMs` 与 `worldUpdateMs`。指标窗口还必须输出事件循环延迟 p50／p95／p99／最大值，以及世界 JSON 字节数、响应 JSON 字节数和状态分区数量等无身份容量指标。
...
- 浏览器运行时测试使用固定 Playwright 版本；CI 与主部署必须优先复用 GitHub hosted runner 已安装的 Chrome／Chromium，并通过 `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` 交给 Playwright，仅在 runner 没有可用浏览器时下载固定 Playwright Chromium。浏览器 CDN 不得成为 CI 或部署的必需单点依赖；无论使用 runner 浏览器还是下载兜底，`npm run test:browser` 都必须继续作为硬门禁。浏览器回归至少覆盖 localStorage 拒绝访问仍能渲染、正式设置控件存在以及无效设置控件不存在；测试 artifact 只在失败时上传并保留 3 天，完整标准输出继续保存在 Actions job log。
...
旧 C2 作业制度 `rapid`／`economical`／`high-yield` 仅在服务器内部作为迁移别名识别，迁移到同一基础产物路线的标准制度后不再公开或接受主动选择。持久化世界版本固定为 31；市场需求模型 19 对新增工业燃料和工业化学品建立直接需求，并在模型 18 升级时释放并重建人口消费与市场储备系统订单，不改写玩家真实资产和成交历史。
