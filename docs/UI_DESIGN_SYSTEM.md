...
主视觉和资产矩阵的非空格子必须复用 `SafeTooltip`，鼠标悬浮或键盘聚焦时显示当前资产，格式固定为“名称 ×数量”；空占位格不显示提示，移动触摸不得增加阻断页面操作的点击层。格子继续保留名称和数量的可访问标签，不得恢复原生 `title`、页面内绝对定位提示或运行时 DOM 扫描。Tooltip 包装层不得改变主视觉 `256px` 上限、正方形比例、工厂插画 `cover` 铺满或数量胶囊层级。
