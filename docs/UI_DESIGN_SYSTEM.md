...
- 宽屏桌面侧栏必须提供键盘可操作的显式折叠按钮；Logo 在折叠前后固定为 `40×40px`，折叠后 Logo 区域通过 hover 与 focus 显示展开箭头并承担展开操作，不得在侧栏边缘另放悬浮按钮。折叠后导航、QQ群和退出按钮仍保留稳定可访问名称，底部两个按钮固定为 `48×48px`；`721px–960px` 继续使用自动紧凑侧栏。
