...
- 当前工厂详情使用唯一 `FacilityClusterInformation` 合并 `4:5` 纵向场景插画、总数量、唯一状态胶囊与 `SwitchControl`；工厂名称只在页面标题显示，内容区不得重复。插画右侧主信息固定为三行：运行中／冻结中／抵押中数量摘要、单厂平均利润、满员率。移动固定头部不得继续承载名称、状态或开关。
