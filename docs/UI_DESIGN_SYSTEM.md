...
- “今日经营”与基础工作入口永久移除。概览正文位于 `building` 面板，只包含签到和三张经营摘要；公开经济事件由 `StrategicWorkspaceChrome` 在页面外的屏幕右栏纵向滚动，绝不回落到概览正文。桌面成长线位于该右栏顶部；移动端右栏隐藏并只保留概览移动成长线入口。
