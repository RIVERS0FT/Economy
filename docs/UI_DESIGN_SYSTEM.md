...
2. **同构列表／表格**：商品、工厂、运输路线、成交、日志、历史记录等主要任务是连续比较多个对象时，使用共享列表或表格行及细线分隔，不为每行重复创建圆角卡片；已登记的建筑两行目录例外使用留白和信息层级而不是可见分隔线区分条目内部区域。
...
- 多条运输路线共享同一州际物理区段时，地图几何不得共线覆盖；按无方向的“州 A—州 B”区段统一计算稳定车道，并沿该区段唯一标准法线对称、紧凑并排，只保留足以区分相邻路线的最小视觉间距。正向、反向和非闭环 `round` 返程分别占用独立车道，创建中的草稿也参与车道分配；同一线路不同运输方式仍共用同一组首府站点，所有保存路线、草稿和高亮的站点圆点固定落在同一首府锚点，并排只作用于首府锚点之间的路线／返程线体，不得为不同运输方式复制或横移站点点位。卡片 hover／focus 高亮必须按路线 ID 精确绑定并复用原路线车道，不得仅按站点序列反查、不得高亮同线路中的另一运输方式，也不得作为新路线再次挤占车道。在途运输标记沿对应路线实际并排后的当前运输段插值。车道几何只在路线集合变化时于静态 SVG 世界坐标系重算，缩放／平移期间仍只复用 `.province-map-camera-surface` 的单一合成变换，不得为避让执行 DOM 测量、高频重投影或相机期重排。
...
- 公路、铁路、航空运输路线必须保持三种不同线型，保存、草稿和高亮状态均不得退化为同一线型；运输地图创建面板不得进入桌面或移动状态栏覆盖区域。同站点序列存在多个运输方式时，路线 hover／focus 必须只高亮当前路线的运输方式和原车道，站点圆点必须共用首府锚点，线路车道保持紧凑并排，不得重新拉开为分散端点。
