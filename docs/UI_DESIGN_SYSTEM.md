...
- 主列只放置“玩家资料”；侧列固定依次放置“游戏设置”“礼品兑换”和“账号与管理”。左右列分别自然堆叠，不得因为另一列高度制造共享行空白。
- 玩家累计统计在宽布局中使用四列，`760px` 以下降为两列；统计块外观继续复用现有 `.player-stat-grid`，不得创建另一套统计卡视觉。
- 昵称编辑在桌面使用输入框和自然宽度“保存昵称”按钮同行；`760px` 以下改为单列且按钮占满可用宽度。
- 当视口不超过 `1180px` 时页面降为单列，顺序固定为“玩家资料／游戏设置／礼品兑换／账号与管理”。列包装器可以使用 `display: contents`，但卡片、标题和表单语义不得丢失。
...
- 把地图缩放恢复为动画帧逐次 `geoRoam`／`setOption`，或在滚轮／双指缩放期间逐帧重绘州面 path、重新投影／重排州名；缩放必须保持共享瞬时等比矩阵并在停止输入且收敛后单次提交正式 ECharts 相机；
