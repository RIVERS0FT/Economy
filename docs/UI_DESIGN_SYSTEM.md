...
管理员入口、游戏入口和九个游戏页面必须使用 `React.lazy` 与动态 `import()` 按需加载；登录页不得静态拉入管理员和全部游戏页面。根游戏模型不得维护每秒变化的时间状态，倒计时只在概览、生产、拍卖、合同和银行等实际需要的局部页面通过共享 `useNow` 维护，市场订单簿、导航和银行资产总览等静态区域不得被全局秒级时钟重渲染。
...
工厂场景插画是经营模拟主视觉，用于让玩家在小尺寸下直接识别产业类型。正式源资源统一保存在 `src/assets/facility-icons/`，文件名必须与服务器 `FACILITY_TYPE_CATALOG` 的 `facilityTypeId` 一致。生产集群选择卡、市场工厂目录卡以及拍卖工厂的大型主视觉、资产矩阵、资产包行和历史图标统一通过 `FacilityIcon` 选择资源；订单表格、本地成交、银行抵押、概览挂单、导航和未知工厂继续使用紧凑 `FactoryIcon`，不得把场景插画扩散到高密度语义位置。
...
当前 C1 复杂度工厂 `farm`、`orchard`、`ranch` 与 `fishery` 已采用统一新风格从空白新绘，均为独立原创构图，不以旧图为编辑、描摹或重绘底稿：农场突出谷仓、粮仓与主食作物，果园突出果树与分拣作业建筑，畜牧场突出主畜舍、围栏和牲畜，渔场突出养殖池、处理建筑与供水设施。四张图都必须在实际 `4:5` 居中裁切后保持主体完整，且不得恢复“底部宽阔道路—远处小建筑—高饱和蓝天”的统一模板。当前批准源图的 SHA-256 与从空白新绘的创作模式记录在 `scripts/facility-artwork-baseline.json`，由 `scripts/verify-facility-artwork.mjs` 校验；替换任一 C1 图片时必须采用独立新构图，同步更新本节视觉规则、机器基线和审核结果，不得只覆盖 PNG 或把旧图作为图像编辑输入。

运行时不得直接加载 1024px 源图。`npm run dev`、`npm run build` 和独立工厂图片验证先调用 `scripts/generate-facility-artwork-thumbnails.mjs`，通过共享预乘 Alpha 面积采样生成 `src/assets/facility-icons/generated/128/` 下的 128px RGBA 缩略图；生成目录是可复现构建产物，构建产物不得提交仓库。`facility-artwork.css` 只能通过 `FacilityIcon` 的 `data-facility-icon` 映射缩略图，并且只允许在上述主视觉上下文显示；未知 ID 沿用组件内厂房 SVG，`prefers-reduced-data` 下所有工厂场景恢复厂房 SVG。生产集群选择卡中的插画必须覆盖完整 `4:5` 竖卡，使用 `background-size: cover` 与居中定位进行等比裁切，不得拉伸、重复、偏移主体或给插画叠加状态滤镜；选择卡必须通过上下两层黑色渐变分别保护顶部名称／利润与底部数量，中央主体区域保持透明，不得用整卡蒙黑替代。运行、异常、停止和选中状态只能由卡片边框、背景、文字和数量表达，不得给插画本身着色。

新增或删除正式工厂时，服务器目录、同名源图、`FacilityIcon` ID 声明、CSS 映射和验证必须在同一变更中完成。`scripts/verify-facility-artwork.mjs` 必须校验目录一一对应、源图、缩略图、映射、生成入口、批准使用上下文、低流量回退、未知 ID 降级，以及 C1 目录与批准源图 SHA-256 基线一致。
...
- 商品标签使用商品 SVG，工厂紧凑标签使用独立厂房 SVG。市场商品与工厂目录卡必须使用两个按 DOM 顺序渲染的覆盖层：图标层分别只包含居中的 `ProductIcon` 或 `FacilityIcon`，数据层包含名称、`CurrencyAmount` 最近成交价、库存／持有数量和真实 DOM“当前”胶囊；桌面中央插画固定 `72 × 72px`，不大于 `720px` 时固定 `56 × 56px`，卡片仍分别保持 `138 × 92px` 与 `132 × 88px`。只允许改变中央插画的 `width`／`height`；图标层 `inset`、数据层 `padding` 与 `gap`、四角槽位、名称前 SVG、价格、“当前”胶囊和库存位置均保持不变，不得通过压缩四角留白容纳插画。名称前固定渲染 `14 × 14px` 对应商品 SVG 或 `FactoryIcon`；商品右下使用 `WarehouseIcon`，工厂右下使用 `FactoryIcon`。目录卡使用 `var(--radius-control)`、`var(--space-3)` 间距、强边框和轻量阴影，悬停不位移。图标层 `z-index` 必须低于数据层，两层均不得截获按钮指针事件。
...
- 第一排固定为“今日经营／本周签到”，桌面按 `5:7` 分栏并统一约 `330px` 高。基础工作按钮保持紧凑，不能占满卡片宽度；经营提醒最多显示 3 条并按仓库、生产、订单、停工、施工排序。
...
- 仓库商品网格使用容器查询。仓库商品网格的列数、容器断点、卡片高度、内边距和图标尺寸唯一归属 `WAREHOUSE_EXPANSION_DESIGN.md` 第 7.1 节；本文件只保留通用视觉与组件边界，不维护第二套几何数值。
...
- 正式导航名称固定为“概览｜市场｜生产｜拍卖｜合同｜银行｜排行｜商店｜设置”。资产总览唯一归属银行，不得恢复独立资产导航。
