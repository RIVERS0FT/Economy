...
- 工厂卡桌面使用至少 320px 的自适应高度，移动端使用自然高度；删除计划区后不得保留固定空白。
...
- 农场使用唯一“种植作物”下拉框；运行中改种时以灰色文字显示下一周期作物，公式仍显示当前作物。
- 固定配方工厂不显示配方区域；不得显示生产模式、目标产量、计划自动保存或保存计划控件。
- 不得恢复独立数量卡、运行描述、下一周期产量辅助句或持续／定量状态小卡片。
- 生产信息、农场作物控件和统一市场入口不得因紧凑布局被删除。
...
- 恢复静态计划模式文本、手动“保存计划”按钮、目标产量输入或其他计划视觉；
- 把生产页主标题恢复为“工厂”；
- 把移动端“紧凑数字”初始状态恢复为关闭；
