...
| `src/styles/production-surface.css` | 生产页共享仓库、建设卡和工厂集群的标题锚点、三行结构与紧凑开关；不得定义一级卡片外层内边距 |
...
- 共享仓库、建设新工厂和工厂集群都是生产页同一一级平面的 `Panel`，统一带 `.production-surface`；它们通过 `primary-surfaces.css` 读取同一个 `--primary-surface-inset`，大于 `720px` 时四边统一为 `16px`，不大于 `720px` 时统一为 `12px`。
