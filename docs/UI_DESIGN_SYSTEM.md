...
运输路线是已登记的独立业务对象例外：每条已保存路线同时承载名称、运行状态、完整站点路径、运输方式、行程、站点数、一次性建线投入和进入独立详情的整体操作，去掉对象边界会让相邻路线的属性归属变得含混。因此一级运输页每条路线固定使用 `.ui-entity-card`，路线卡之间只使用共享 `gap` 分隔，不绘制行分割线；该例外不得扩展到商品目录、工厂目录、成交或历史记录。运输页固定底部“增加路线”属于页面内 sticky 操作区，不使用毛玻璃或浮动阴影，不显示路线数量／上限胶囊，并必须始终位于当前页面承载面和移动安全区内。
...
正式商品 `industrial-fuel`、`industrial-chemicals`、`fertilizer` 和正式设施 `fertilizer-factory` 必须分别具有 1024×1024 RGBA 源图、运行时缩略图、共享 `ProductIcon`／`FacilityIcon` ID 与 CSS 映射。工业燃料主视觉固定使用红色钢制燃料桶与清晰易燃标志，紧凑 SVG 同步使用红色危险语义；工业化学品主视觉使用密封蓝色工业化学品桶、实验器皿与分子结构，不得出现肥料袋、叶片、土壤或颗粒等农业语义，也不得与化肥复用同一源图。化肥主视觉继续使用敞口颗粒肥料袋、叶片和紧凑土壤垄沟语义；化肥厂当前批准构图以中央造粒塔、双侧储罐、输送管廊和装袋区为核心，并与以成组分馏塔为主体的炼油厂保持可辨识差异。目录外回退不得替代正式资产；化肥厂插画哈希必须进入现有工厂基线，三种商品源图继续接受商品资源完整性与互异性验证。
