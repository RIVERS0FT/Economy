...
- `FactoryIcon` 使用厂房与烟囱轮廓，`machinery` 商品继续使用 `ProductIcons.tsx` 中的齿轮机械轮廓，两者不得共用路径；
...
- 仓库商品卡使用 `ProductIconLabel`，并按“图标与名称／库存总量／可用与冻结”的三层紧凑结构展示。
...
- 不显示固定副标题“市场交易版”。
- 不恢复独立玩家头像或重复用户名卡。
- 正式导航名称固定为“概览｜市场｜生产｜资产｜排行｜设置”，不得把 `assets` 的显示名称恢复为“资金”。
...
- 在商品标签、商品行情和仓库商品卡中恢复字符占位图标或无图标文本卡；
- 让工厂标签恢复齿轮、`⚙` 或与机械商品相同的 SVG 路径；
- 让导航图标或文字恢复半透明 RGBA、`opacity < 1` 或未选中图标的独立透明色；
- 恢复仓库商品筛选、全部商品视图或资产页逐商品库存与估值卡；
- 让移动状态栏或底栏忽略安全区；
- 给导航活动态添加位移或缩放；
- 使用运行时 DOM 扫描、文本匹配或 `MutationObserver` 修补已渲染页面；
- 未更新本文档和验证脚本就改变令牌、断点、共享组件、图标体系或关键布局。
## 工厂卡桌面固定高度与控件规则（2026-07-12）

工厂卡桌面固定高度为 384px，移动端使用自然高度。开关继续使用唯一 `SwitchControl`／`.ui-switch`，并固定在卡片右上角。异常原因必须写入状态标签，不得通过开关变体表达。

计划编辑采用同一行的摘要、下拉框、可选数字输入和统一“保存计划”按钮，不得新增“持续”“定量”状态小卡片或另一套开关样式。
