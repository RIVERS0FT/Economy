...
生产设置中的“生产产物／作业制度”是共享 `RichSelectInput` 唯一批准的 `production-config` 变体。该变体参考经营模拟游戏的生产方式切换信息层级，以“生产方案槽”承载当前选择；收起触发按钮只显示当前产物／作业制度图片，不显示名称、参数摘要或下拉箭头，图片槽不得绘制独立黑色底板；展开后仍使用同一 `combobox`／`listbox`、同一 `open`／`activeValue`、文字快速定位、键盘导航、焦点返回、工作区／Dialog Portal、顶层 Popover 与刷新透明性内核，不得复制第二套 Popover、键盘导航或刷新状态。`production-config` 只允许在 `form-controls.css` 中改变共享外观：触发槽可高于默认控件，方案行可高于默认选项，菜单允许宽于触发器，但宽度必须收敛在当前工作区或根级 Dialog 可用矩形内；设置、合同、拍卖、建设工厂等默认下拉不得继承这些尺寸。
...
- 玩家可见的“生产产物”与“作业制度”固定使用同一个“生产设置”区和共享 `RichSelectInput` 的 `production-config` 生产方案槽；桌面和移动详情都固定双列同行，移动端在 `320px` 及以上不得因容器不大于 `479px` 改为单列。两个收起触发按钮都只显示当前产物／作业制度图片，不显示名称、参数摘要或下拉箭头，图片本身不得带独立黑色底板或图片槽边框；展开菜单才显示候选名称、结构化投入／产出、周期／成本信息，作业制度候选还必须相对当前方案标示周期、成本与产量变化。不得恢复收起态文字详情、箭头或作业制度说明。
...
- 把生产产物／作业制度收起态恢复为“图标 + 名称／参数／箭头”、恢复图片黑色底板或图片槽边框、改回默认普通下拉、删除 `production-config` 生产方案槽、把方案菜单重新限制为半宽触发器宽度，或在业务组件复制第二套 Popover、键盘导航、焦点返回、Portal 或刷新状态；
...
进行中资产包卡最多直接展示前三项，使用“当前总价”和“整包出价”；“整包出价”的 `MoneyInput` 与“提交出价”／“提高出价”按钮在所有断点保持同一操作行，输入列弹性收缩、按钮保持自然宽度且不换行，不得在窄卡或移动端把按钮折到下一行；多项视觉采用本地图标网格，不使用字符占位。零场空状态只使用一层表面，不得渲染最近结束或历史结算区域。
...
- 控件收起时只显示当前方案图片，不显示名称、参数摘要或下拉箭头；图片槽背景透明且无独立边框，完整名称与参数只在展开候选中显示；不在生产设置下方恢复独立“周期 · 产出 · 成本”摘要，不重复当前制度名称，不显示作业制度说明。
