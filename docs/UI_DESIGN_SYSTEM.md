...
生产设置中的“生产产物／作业制度”是共享 `RichSelectInput` 唯一批准的 `production-config` 变体。该变体参考经营模拟游戏的生产方式切换信息层级，以“生产方案槽”呈现当前图标、名称和紧凑参数；展开后仍使用同一 `combobox`／`listbox`、同一 `open`／`activeValue`、文字快速定位、键盘导航、焦点返回、工作区／Dialog Portal、顶层 Popover 与刷新透明性内核，不得复制第二套 Popover、键盘导航或刷新状态。`production-config` 只允许在 `form-controls.css` 中改变共享外观：触发槽可高于默认控件，方案行可高于默认选项，菜单允许宽于触发器，但宽度必须收敛在当前工作区或根级 Dialog 可用矩形内；设置、合同、拍卖、建设工厂等默认下拉不得继承这些尺寸。

生产产物的方案槽使用 `ProductArtwork` 和路线名称，紧凑摘要显示当前实际产出、数量与周期；候选方案菜单必须直接展示当前作业制度下该路线的实际投入、产出、周期与周期成本。作业制度方案槽使用统一功能 Icon 和制度名称，紧凑摘要显示周期、成本与产量；候选方案必须读取当前产物对应的 `plansByRecipeId`，相对当前方案标示周期、成本与产量变化，并同时显示实际投入品。绿色／红色只表达“该指标相对当前更有利／更不利”的方向，不得把高速、高产或任何制度整体承诺为盈利；真实利润继续由生产结算和真实成交价表达。当前方案在菜单中使用共享选中标记，作业制度说明不得显示。

移动工厂详情中的生产产物与作业制度继续同一行双列；打开 `production-config` 时，方案菜单可以扩展到根级 Dialog 的可用宽度而不受半宽触发槽限制，必须保持在视口内且不得制造横向溢出。根级 Dialog 内的 `RichSelectInput` 列表继续复用该 Dialog 根作为安全定位边界并位于详情遮罩之上。`production-methods.css` 只负责投入／产出／指标摘要的业务内部排列，不得定义触发器、弹层、定位、键盘或选中基础视觉。
...
- 玩家可见的“生产产物”与“作业制度”固定使用同一个“生产设置”区和共享 `RichSelectInput` 的 `production-config` 生产方案槽；桌面和移动详情都固定双列同行，移动端在 `320px` 及以上不得因容器不大于 `479px` 改为单列。生产产物槽使用产出商品 `ProductArtwork`、路线名称和当前产出／周期紧凑摘要；作业制度槽使用统一功能 Icon、名称和当前周期／成本／产量紧凑摘要。展开菜单必须展示候选方案的结构化投入／产出与周期／成本信息，作业制度候选还必须相对当前方案标示周期、成本与产量变化；不得恢复仅“图标 + 名称”的普通生产下拉，也不得显示作业制度说明。
...
- 把生产产物／作业制度恢复为默认仅“图标 + 名称”的普通下拉、删除 `production-config` 生产方案槽、把方案菜单重新限制为半宽触发器宽度，或在业务组件复制第二套 Popover、键盘导航、焦点返回、Portal 或刷新状态；
...
- 控件收起时只显示当前方案的图标、名称和紧凑参数摘要；不在生产设置下方恢复独立“周期 · 产出 · 成本”摘要，不重复当前制度名称，不显示作业制度说明。
