...
- 移动底栏导航角标使用 `.navigation-badge`，颜色固定沿用 `var(--color-success)` 背景和 `var(--color-on-primary)` 文字。每个页面最多一个合并数字，只显示 `1`～`99` 或 `99+`；桌面侧栏不渲染角标，但必须在按钮 `aria-label` 中保留完整数量和来源。移动角标固定在按钮内部右上角，不得裁剪数字。
...
- 桌面通知面板在工作区右上角展开，移动通知面板在状态栏下方、底部导航上方展开。面板使用普通高不透明度深色表面、单一纵向滚动区和工作区安全浮层；面板外的点击捕获层保持完全透明，不得压暗常驻地图。不新增毛玻璃宿主、根级 Dialog 或页面内嵌卡片。
...
- 面板顶部直接对齐工作区浮层起点，复用状态栏与工作区之间已经存在的唯一沟槽，不得再叠加第二个顶部 `var(--layout-gutter)`；左右和底部继续使用 `var(--layout-gutter)`。点击面板外遮罩空白、按 `Escape` 或点击关闭按钮都必须关闭，面板内部 Pointer 操作不得冒泡触发关闭。关闭后焦点返回通知入口；入口使用 `aria-expanded`／`aria-controls`，面板使用命名 `dialog`，删除按钮必须包含具体通知标题的可访问名称。
