...
银行页必须使用 `PageLayout` 和 `PagePanel`，一级顺序固定为资产总览、存款账户与存款利息双列、工厂抵押贷款、银行记录。`src/styles/asset-overview.css` 只负责资产总览内部几何，`src/styles/bank.css` 只负责存款余额条、业务网格、紧凑工厂表格、抵押胶囊、还款与银行记录的业务布局；不得恢复独立资产页或页面顶部五张重复指标卡。输入、按钮、开关、状态标签、数据行、货币图标、一级面板外层内边距、hover 和焦点继续由共享设计系统收束。
...
Playwright 必须验证 `1684×931`、`1280×900` 和 `900×1000` 下的真实卡片位置、动态行情高度、ECharts SVG 初始化、零涨跌中性状态、禁用原因、资产目录滚动、订单簿标题顺序、同价档位聚合及水平溢出；还必须在桌面和移动宽度验证市场工厂插画与卡片四边重合、`background-size: cover`、居中定位、上下渐变和数据层覆盖关系。市场专项断点继续以 `MARKET_CHART_LAYOUT_DESIGN.md` 为准。
