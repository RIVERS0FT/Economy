...
- 让生产公式恢复单座输入、输出或成本，或让运行中的公式不同时显示 `participatingCount`、周期完成时满员率和 `cycleEffectiveCount`；
- 停止或异常使用 `productionAvailableCount`、实时投影的满员率和 `staffingBatchCarryBps` 计算启动后或恢复后的整数等效产能；不得使用 `group.count` 作为公式乘数，也不得把 `pendingJoinCount` 提前计入当前周期；
