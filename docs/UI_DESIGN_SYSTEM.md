...
- 未知商品 ID 使用统一包装箱 SVG 回退，不得隐藏商品。

当前 12 种正式商品必须各有独立本地内联 SVG：粮食、木材、铁矿石、原油、面粉、木板、钢材、塑料、食品、家具、机械和电子产品。
...
- 开关固定在卡片右上角。
- 异常原因只在顶部状态标签显示一次。
- 顶部数量摘要只显示“运行中／下一周期加入／冻结中”。
- 停止和异常状态使用紧凑状态行，不显示大型空进度条。
- 计划编辑采用当前计划摘要、模式下拉框、可选定量输入和统一“保存计划”按钮。
