...
| `src/styles/production-surface.css` | 生产页共享仓库、建设卡和工厂集群同一一级平面的统一内边距与标题锚点 |
...
- 一级卡片标题元素盒子的左上锚点必须一致；右侧等级标签、状态标签或 `SwitchControl` 的 `44 × 44px` 点击区域不得改变标题位置。容器查询只允许调整卡片内部密度和换行，不得改变一级外层内边距。
...
- 标题文字不得因 `SwitchControl` 的 `44 × 44px` 点击区域而垂直居中；标题行内容顶部对齐，名称可见文字从统一卡片上内边距开始。
...
- 把建设卡恢复为桌面独占整行；
- 让共享仓库、建设新工厂和工厂集群在同一断点使用不同一级外层内边距，或让容器查询按卡片自身宽度改变一级 `padding`；
...
- 把标题行恢复为由 `44 × 44px` 开关点击区域垂直居中，导致名称顶部留白大于左侧留白；
