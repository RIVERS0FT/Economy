...
| `src/styles/icon-system.css` | 全局 SVG 图标尺寸、导航图标槽位和移动图标尺寸 |
...
所有应用界面图标必须来自 `src/components/icons/GameIcons.tsx`：

- 状态栏和桌面／移动导航不得继续使用 Unicode 字符、Emoji 或字体符号作为图标；
- 所有图标使用统一 `24 × 24` `viewBox`、`currentColor`、圆角端点和约 `1.9` 描边；
- SVG 根节点必须带 `.game-icon`，尺寸由 `src/styles/icon-system.css` 统一控制；
...
- 品牌 Logo、商品图片和工厂插图属于内容图像，不受本节“界面图标”规则限制。
