...
> 状态：当前共享仓库容量、占用、升级与页面归属基线  
> 适用项目：`RIVERS0FT/Economy`  
> 更新时间：2026-07-11  
> 客户端状态版本：`version: 7`

页面内容与模块归属以 `docs/PAGE_CONTENT_AND_NAVIGATION_DESIGN.md` 为准。共享仓库完整管理入口固定在“生产”导航对应的 `ProductionPage`。
...
13. `ProductionPage` 使用共享扩容组件；
14. 资金页和设置页不渲染共享扩容组件；
15. 状态栏显示买单预占；
16. 本地日志记录等级和容量变化；
17. 页面归属设计明确仓库唯一完整管理入口。
...
- 在页面中复制另一套容量或费用公式；
- 忽略未完成买单对仓库容量的预占；
- 把 `WarehouseUpgradeCard` 移回资金页或设置页；
- 在多个页面重复提供扩容入口。
