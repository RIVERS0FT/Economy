...
- `EconomicEventMapLayer` 只位于现有权威 `.province-map-world-svg` 内，复用同一投影、Camera 与 active 栅格快照；不得创建第二地图、HTML 屏幕固定事件按钮条或第二投影。地区事件使用当前正式州目录的首府坐标；全国事件使用固定全国展示锚点，锚点不改变生效范围。公共项目从同一项目 ID 的服务器视图取得项目州，缺少合法地点时不伪造落点。
- 事件标记有独立 `44px` 命中范围、键盘焦点和可访问名称。预告使用虚线轮廓，进行中使用实线轮廓，不只依赖颜色区分。地图选路期间隐藏事件点，让既有州面输入独占选路行为；点击／键盘激活事件只打开详情，不穿透选择州或触发地图双击复位。
- 重叠标记按事件 ID 确定性排布，以屏幕空间间距避让并用引线连接真实世界锚点；不得随机抖动或按数量截断。屏幕尺度只在既有 Camera idle／settled 边界与容器 resize 更新；事件时钟仅在叶子内运行，开始、结束、列表或尺度真实变化才使栅格快照失效。不得把计时文字放进每秒更新的地图栅格、每帧读取 DOM、修改 Camera RAF 或新增第二套手势状态。
- 插画及弹窗视觉归 `UI_DESIGN_SYSTEM.md`，内容及生命周期归 `PAGE_CONTENT_AND_NAVIGATION_DESIGN.md`。纯逻辑回归覆盖坐标缺失、同点避让、数量超过四条、预告／到期与公共项目；浏览器回归覆盖同一 SVG、缩放后命中尺寸、事件移除、焦点和镜头保持。
