...
共享结算的物资槽与商业收入槽保持同一卡片几何、内边距、高度和局部圆角；商业收入只显示金额组件自身的一个资金图标，不得在外层叠加第二个资金图标。资金底座、资金图标和资金文字以投入资金数据带为视觉基准：商业收入资金卡与周期／成本数据带共用同一资金表面（边框、背景和内阴影），资金图标固定同一尺寸，金额文字与图标统一使用投入资金颜色；商品产出槽仍使用商品产出语义，不得把商品产出色重新应用到资金卡。周期与成本继续保留在同一条操作数据带中，不拆成两个卡片；该数据带与结算物资／收入槽使用同一局部圆角，并保留周期与成本之间的竖向分隔线。
