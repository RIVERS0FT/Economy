...
服务器保存权威经济状态，本地日志仅由当前浏览器比较相邻权威状态生成，不参与资产、银行存贷款、利息、抵押、仓库、工厂、生产、订单、产权、估值或排名。同一用户的本地文档必须在页面生命周期内缓存，滚动或状态轮询不得反复从 `localStorage` 解析全部记录。持久化写入通过 `requestIdleCallback` 或定时回退合并，并在 `pagehide` 时刷新；没有新事件或成交时不得重写完整文档。订单和挂牌差异使用字段比较，不得以整数组 `JSON.stringify` 作为每次轮询的变化检测。

本地日志可以记录相邻快照可见的可用／冻结资金、库存、仓库、工厂集群、生产、下单、撤单和本人订单成交，但普通玩家只能感知自己的订单完成情况，不得知道成交去向、来源或对手身份。
...
快照只保存生成展示差异所需字段：可用资金、冻结资金、库存、仓库、工厂集群、施工、本人订单及匿名 fills、商品目录和工厂目录。银行存款、贷款、利息池、抵押明细与权威银行流水不进入本地快照；银行动作产生的可用资金差异可以作为普通资金事件显示，但完整存取款、付息、结息、宽限和违约记录只读取银行页服务器最近记录。
...
- 资产页显示可用／冻结资金、商品、仓库、工厂和生产变化；银行权威流水固定显示在银行页，不复制为本地记录。
- 概览页最多显示最近 6 条匿名成交。
- 所有界面必须标注“本地记录”。
...
- 把银行存款、贷款、抵押、利息池、微单位余数或权威银行流水保存为浏览器本地权威快照；
- 根据本地现金差异推断利息来源、贷款余额、抵押物或违约结果，或用本地日志恢复银行状态；
