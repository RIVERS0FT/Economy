...
48. 正式世界调度只能使用 `world-deadline-planner.js` 计算的单一最早到期 `setTimeout`，不得恢复固定一秒 `setInterval` 或在空闲窗口反复克隆、迁移、深比较和写入世界；工厂即时建设不得注册施工完成或施工就业截止时间，研发、生产、合同、拍卖、银行和排行榜等真实权威截止事件继续由统一规划器选择最早到期；动作前后处理顺序保持不变，并通过 `server/test/world-deadline-planner.test.js` 与 `scripts/verify-runtime-efficiency.mjs` 防回退。
