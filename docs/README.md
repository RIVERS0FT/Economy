...
工厂建设以服务器正式目录的 `buildCost + buildInputs` 为唯一成本，新玩家首座工厂材料包固定为 4 木材与 2 铁矿石，既有空白玩家只迁移补发一次；在一个幂等事务中原子扣除后立即增加同类集群数量；不得恢复施工时间、施工任务、施工队列、施工倒计时或工厂宝石加速。规则变更必须同步更新产业、产品、页面、服务器、宝石与权威倒计时文档，以及目录、宝石、倒计时和服务器测试。
