...
43. 统一导航角标的单数字绿色视觉、`99+` 上限、拍卖／合同按业务 ID 合并去重、排行榜结算已读基线和页面启用范围属于页面与 UI 共同规则；必须同步更新 `PAGE_CONTENT_AND_NAVIGATION_DESIGN.md`、`UI_DESIGN_SYSTEM.md`、共享角标组件、浏览器本地已读状态、`scripts/verify-navigation-badges.mjs` 与浏览器测试，不得恢复市场专用、双项或多项角标。
