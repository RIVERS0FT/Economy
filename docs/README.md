...
73. 世界 30 的 34 个省级地区、本地无限仓库、本地商品／工厂行情、省级工厂集群与订单隔离属于产品、产业、仓库、订单簿、页面、UI、拍卖和服务器共同规则。旧数据统一迁入北京且必须资产守恒；现金、宝石、研发、银行、排行榜与世界人口不按地区复制；当前版本不得增加免费跨地区物流或让地区切换移动资产。实现必须同步 `shared/provinces.json`、客户端状态版本 34、地图页、写动作 `provinceId`、专项服务器测试与 `scripts/verify-provincial-economy.mjs`。
