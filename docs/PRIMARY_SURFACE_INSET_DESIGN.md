...
- `GlobalBuildingsPage` 等全局经营页必须以 `minmax(0, 1fr)`、`min-width: 0` 和可收缩的紧凑列表行保证内容随真实承载面收缩。建筑页已退役顶部四项汇总；全局工厂目录和地区建筑入口均以纵向列表作为安全几何目标。全局工厂目录在移动端仍保持“工厂｜平均利润／分钟｜拥有”的单行表格结构，只能缩小正方形插画、字号、列间距和列最小宽度；地区建筑入口可把四项状态指标折成两列。两者都不得依赖卡片网格、抽屉裁剪或页面级横向滚动。
- 全局建筑列表的响应不能只依赖浏览器 viewport。`.global-operation-page` 必须建立 inline-size 容器；真实页面承载宽度不大于 `620px` 时，全局工厂目录通过更紧凑的单行列宽和 `40×40` 正方形插画收缩，极窄 `360px` 及以下可进一步缩至 `36×36`；真实承载宽度不大于 `520px` 时，地区建筑入口把四项状态指标折成两列。原因是 `721px` 及以上已经进入桌面外壳，但固定侧栏和主卡轨道仍可能把实际页面槽压缩到远小于移动断点的宽度；响应必须以实际页面槽为准，同时不能为了避免溢出破坏全局工厂单行语义。
...
- 全局建筑页的全局工厂目录与地区建筑入口使用可收缩纵向列表；全局工厂目录在 `620px`、`360px` 实际承载断点继续保持单行并缩小插画与列宽，地区建筑入口在 `520px` 实际承载断点折为两列指标，两者都不得产生页面级横向溢出，也不得恢复已退役的 `.global-operation-metrics`；
- 样式加载顺序正确；
- 已清理页面不再声明旧外层 padding；
- 本设计文档中的唯一权威和禁止回退规则仍存在；
- `tests/browser/player-page-geometry.spec.ts` 在桌面和移动断点覆盖正式页面，验证页面不超出承载面、可滚动正文首项有统一顶部间距且根级无横向溢出，并对全局建筑两类纵向列表执行边界与跨断点真实几何回归；其中全局工厂条目必须在所有覆盖断点保持单行高度；
