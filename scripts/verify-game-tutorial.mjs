...
requireText(gameShell, "const showRightRail = pagePresentation !== 'fullscreen';", '全宽页面必须统一关闭右侧信息栏');
requireText(gameShell, 'tutorial={showRightRail ? tutorial : undefined}', '全宽页面隐藏教程只能发生在外壳展示层');
forbidText(gameShell, 'tutorial.skip()', '页面切换不得把隐藏教程实现成跳过');
...
requireText(tutorialBrowser, "page.locator('.strategic-economic-event-rail')).toHaveCount(0)", '浏览器回归必须验证全宽页面无右栏 DOM');
requireText(tutorialBrowser, "page.locator('.game-guide-strip')).toHaveCount(0)", '浏览器回归必须验证全宽页面无教程 DOM');
requireText(tutorialBrowser, "page.locator('.economic-event-log-panel')).toHaveCount(0)", '浏览器回归必须验证全宽页面无事件 DOM');
requireText(tutorialBrowser, "getByText('步骤 1/9', { exact: true })", '离开全宽页面后教程必须恢复同一步骤');
...
requireText(pageDesign, '桌面教程显示在外壳右侧信息栏顶部，只由进行中的本地教程轮次控制', '页面权威设计必须锁定教程外壳归属');
requireText(pageDesign, '所有 `fullscreen` 页面统一不挂载右侧信息栏', '页面权威设计必须锁定全宽页面教程显示例外');
requireText(pageDesign, '该隐藏只改变展示，不修改教程轮次、当前步骤、跳过状态或完成状态', '页面权威设计必须锁定全宽隐藏不等于跳过');
...
requireText(chromeDesign, '所有 `fullscreen` 页面进入后整个右侧信息栏不挂载', '外壳权威设计必须锁定全宽页无右栏');
