...
for (const text of [
  ...
  'BANK_DAILY_INTEREST_CAP_BPS = 25',
...
for (const text of [
  ...
  'deposit interest is paid only from realized borrower interest and new deposits wait a full day',
  'loan assessment exposes transparent collateral and rate inputs',
]) requireText('server/test/banking.test.js', text);

for (const text of ['<BankPage model={model} />', 'version: 21']) requireText('tests/browser/bank-runtime-harness.tsx', text);
...
console.log('银行验证通过：存取款守恒、抵押生产边界、净资产、透明额度、期限利率、真实利息池、每日最低余额结息和统一截止时间均已锁定。');
