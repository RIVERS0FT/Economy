...
assert.equal(ticks[1].timestamp - ticks[0].timestamp, 2 * 60 * 60 * 1000, '横轴每段必须覆盖 2 小时');
...
for (const text of [
  ...
  'Math.max(48, rootFontSize', '(0.22 / 0.78)',
...
for (const text of ['LineChart', 'BarChart', 'PieChart', 'GridComponent', 'TooltipComponent', 'AriaComponent', 'SVGRenderer']) {
...
for (const text of [
  ...
  'data-echarts-ready', 'volumeHeight', 'volumeShare', 'legendCenterDelta',
  '成交量图区实际高度不得低于 48px', '成交量图区不得低于数据绘图区的 22%',
]) assert.ok(safeZoneSpec.includes(text), `行情图浏览器几何回归缺少: ${text}`);
for (const text of ['priceTicks', 'volumeTicks', 'ECharts SVG is not ready', 'market-chart-footer']) {
...
for (const text of [
  '市场行情图几何与可读性唯一专项基线', 'ECharts SVG', '双 Grid',
...
console.log('Market ECharts verification passed: dual-grid SVG rendering, integer axes, dynamic readable volume geometry, stable lifecycle and safe-zone regressions satisfy the design baseline.');
