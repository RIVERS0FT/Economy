...
for (const text of [
  'MARKET_DEMAND_MODEL_VERSION = 15',
...
for (const text of [
  ...
  'population model 6 migration refunds current model 15 escrow before rebuilding demand',
...
for (const [path, texts] of [
  ['docs/PRODUCT_AND_GAMEPLAY_DESIGN.md', ['市场需求模型版本：15', '单座 C1 工厂人口承载基数固定为 **11**', '每五分钟迁入剩余缺口的 **2%**', '实际人口 × 0.57', '三类人口账户', '`lavish` 奢靡', '自动稳定补充发生前', '状态只重新分配同一周期预算', '真实冻结资金', '稳定需求补充', '三周期目标钱包', '双向报价锚点', '上一锚点的 0.75%', '参考价缺口的 5%', '最多为参考价的 2%', '只恢复 2.5% 缺口', '当前报价锚点上追涨 0.5%']],
  ['docs/UNIFIED_ASSET_ORDER_BOOK_DESIGN.md', ['市场需求模型版本：15', '`populationModelId`', '`fundingPool`', '真实人口冻结资金', '双向报价锚点']],
  ['docs/SERVER_ARCHITECTURE_AND_DEPLOYMENT_DESIGN.md', ['population-economy.js', 'population-demographics.js', '人口经济内部版本固定为 7', '五档状态只重新分配食品／家庭与类别份额', '市场需求模型 15', '人口消费不得发行普通货币']],
...
console.log('市场需求验证通过：模型 15 使用工厂承载驱动的实际人口与真实钱包覆盖全部 33 种商品，并保持双向报价、派生流动性和市场储备约束。');
