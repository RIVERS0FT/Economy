...
for (const text of [
  ...
  "ownerName: '饮食需求'", "ownerName: '家庭用品需求'", 'previousVersion >= 12', 'previousVersion < 12', 'nextDemandAt: now',
]) assert.ok(domain.includes(text), 'domain.js 缺少: ' + text);
const assetEvents = read('server/src/asset-events.js');
assert.ok(assetEvents.includes('world.version = 12;'), '日志清理器必须保留世界版本 12');
assert.equal(assetEvents.includes('world.version = 11;'), false, '日志清理器不得把世界版本降回 11');
...
for (const text of [
  ...
  'world version 11 migration immediately rebuilds current-cycle population demand',
...
for (const [path, texts] of [
  ...
  ['docs/SERVER_ARCHITECTURE_AND_DEPLOYMENT_DESIGN.md', ['世界版本 11 升级到 12', '不得把世界版本写回旧值']],
]) {
  const content = read(path);
  for (const text of texts) assert.ok(content.includes(text), path + ' 缺少: ' + text);
}
console.log('人口需求验证通过：两类人口需求预算提高至 500／480，并按生产链双向滞后传导价格。');
