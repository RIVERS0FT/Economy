...
assert.match(browserTest, /toast shares the event rail layer at bottom-right/);
assert.match(browserTest, /desktop toast remains bottom-right when the public event rail is hidden/);
assert.match(browserTest, /toastStackZIndex\)\.toBe\(geometry\.eventRailZIndex\)/);
assert.match(browserTest, /Math\.abs\(geometry\.toast\.width - geometry\.eventRail\.width\)/);
