...
text = text.replace('每项商品保存六位小数报价锚点', '每项商品保存双向报价锚点（六位小数）')
...
product_text = product_text.replace(
    '锚点合法范围为 0.01 至基础价的 300%；零成交、成交不足、持续过剩和短缺追涨仍使用既有平滑规则。',
    '锚点合法范围为 0.01 至基础价的 300%；上一完整周期存在直接需求且零成交时，上涨量取“上一锚点的 0.75%”与“参考价缺口的 5%、但最多为参考价的 2%”中的较大值；存在成交但直接成交率低于需求组目标时，锚点低于参考价只恢复 2.5% 缺口，高于参考价保持；普通达标、无需求和持续过剩仍双向回归或下调。压力达到 1.15 时，参考价 103% 仅作为短缺目标，每周期最多在当前报价锚点上追涨 0.5%。',
)
