...
def replace_line(text: str, prefix_pattern: str, replacement: str, label: str) -> str:
    return replace_once(text, rf'^{prefix_pattern}.*$', replacement, label)
...
old_map_rule = '地图使用共享 `EconomyChart` 的 ECharts Geo/Map 并位于铺满视口的根级地图层，州面点击直接切换地区；'
new_map_rule = '地图使用独立静态 SVG 世界面与单一 `.province-map-camera-surface` 合成相机并位于铺满视口的根级地图层，州面点击直接切换地区；缩放／平移不得恢复 ECharts Geo/Map、`geoRoam`、第二套标签相机或手势期间几何重建；'
if readme.count(old_map_rule) != 1:
