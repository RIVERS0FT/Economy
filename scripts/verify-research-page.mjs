...
for (const text of [
  ...
  '按技术领域选择科技',
  "technology.branch ? branchLabels[technology.branch] : '技术研发'",
  "解锁商业建筑",
...
for (const text of [
  ...
  "getByText('18 项科技', { exact: true })).toHaveCount(0)",
  "page.locator('.page-card-scroll-area')).toHaveCount(0)",
  "page.locator('.page-card-static')).toBeVisible()",
  "page.locator('.research-technology-node .research-facility-artwork')).toHaveCount(18)",
  "page.locator('.research-technology-node .research-technology-node-name')).toHaveCount(18)",
...
for (const text of [
  'C1-C7 只表示科技阶段',
  '节点按真实 prerequisiteTechnologyIds 布局',
  '自上而下 DAG',
  'C1–C7 仍作为科技定义中的知识阶段元数据',
  '也不得作为科技节点可见字段',
