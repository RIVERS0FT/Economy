...
function parsePng(filePath, sourceSize) {
  ...
  if (inflated.length !== expectedLength) throw new Error(`${filePath} PNG 解压长度无效`);
...
function downsampleWithPremultipliedAlpha({ width, height, pixels }, targetSize) {
...
function encodePng(pixels, targetSize) {
...
export function generateArtworkThumbnails({
  ...
  const outputDirectory = resolve(sourceDirectory, `generated/${targetSize}`);
  ...
  let totalBytes = 0;
  for (const id of ids) {
    const sourcePath = join(sourceDirectory, `${id}.png`);
    const outputPath = join(outputDirectory, `${id}.png`);
    const thumbnail = encodePng(
      downsampleWithPremultipliedAlpha(parsePng(sourcePath, sourceSize), targetSize),
      targetSize,
    );
  ...
  console.log(
    `${label}运行时缩略图生成完成：${ids.length} 种 ${targetSize}×${targetSize} RGBA PNG，`
