...
for (const path of versionedDocs) {
  ...
  if (!content.includes('世界状态版本：24')) failures.push(`${path} 世界状态版本必须为 23`);
}
...
console.log(`文档权威性验证通过：精简协作入口与项目 README、登记文档清单、版本 ${CURRENT_CLIENT_STATE_VERSION}/23、500 新玩家启动资金、市场需求模型 14、固定银行收益与周资金结算、长期生产合同、商品／工厂资产拍卖、市场行情图可读性、真实人口钱包、就业资金流、统一订单簿、双边市场储备和十页导航与银行资产总览职责均满足当前基线。`);
