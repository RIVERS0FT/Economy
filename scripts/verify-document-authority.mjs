...
if (existsSync(pathFor('README.md'))) {
  ...
  for (const text of [
    ...
    '真实资金和库存同时生成商品买单与卖单',
    '模型 4 升级到 5 时删除旧系统商品订单、释放储备冻结资金和库存',
...
console.log('文档权威性验证通过：登记文档清单、未知 Markdown 拒绝、版本 15/13、市场需求模型 5、非交叉统一订单簿、双边市场储备、九页导航、商店、整数经济基线、权威倒计时、一级卡片外层内边距、外壳与滚动条归属、单一职责专题、文档整理规则和旧文件禁令均满足当前基线。');
