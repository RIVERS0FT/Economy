...
if (!existsSync(pathFor(agentsPath))) {
  ...
  for (const text of [
    ...
    '只保留当前最终规则和必要的 `non-obvious reason`',
    '不得保留 `intermediate attempts`',
    'PR 描述只记录合入后的最终行为',
    'diff 看不出来的取舍',
    '从未合入的状态',
...
for (const path of versionedDocs) {
  ...
  if (!content.includes('世界状态版本：32')) failures.push(`${path} 世界状态版本必须为 32`);
}
...
console.log(`文档权威性验证通过：精简协作入口与项目 README、登记文档清单、版本 ${CURRENT_CLIENT_STATE_VERSION}/32、美国本土连续 48 州、起始州与州解锁、三种跨州运输、本地市场与工厂、500 新玩家启动资金、市场需求模型 19、固定银行收益与周资金结算、商品供货、玩家抵押借贷与工厂使用权租赁合同、商品／工厂资产拍卖、市场行情图可读性、真实人口钱包、就业资金流、统一订单簿、双边市场储备、官方系统价市场、十二个正式页面与十一项可见导航和银行资产总览职责均满足当前基线。`);
