...
requireText('server/src/commercial-contracts.js', ["'loan'", "'facility_lease'", 'MAX_LOAN_TO_VALUE_BPS', 'COMMERCIAL_GRACE_MS', 'transferableFacilityQuantity', 'ensureBankWorld', 'settleLoanDefaultWithBank', 'bankGuaranteedCredits', 'bankCollectedCredits', 'bankCollateralSeizedQuantity', 'lastCompensationFromId']);
...
requireText('server/src/contract-audit-store.js', ['player_loan_collateral_release', 'player_loan_collateral_remainder_release', 'player_loan_bank_guarantee', 'player_loan_bank_collection_deposit', 'player_loan_bank_collection_cash', 'player_loan_bank_collection_collateral', 'loan_bank_guarantee_settled', 'lease_usage_right_return', 'contract_default_confirmed', 'contract_default_claimed', 'loan_default_confirmed', 'loan_default_claimed', 'lastCompensationFromId', 'lastCompensationToId']);
...
requireText('src/pages/ContractWorkspacePage.tsx', [
  ...
  'bankSettlingLoan', '银行清算中', '等待银行清算', '解除合同并领取违约金',
  'Number(isConfirmedDefault(b))', 'canClaimConfirmedDefault', 'defaultClaimLabel',
...
console.log('六类合同方向、资产锁定、玩家借贷银行担保清算、现行工作区入口与权威设计验证通过');