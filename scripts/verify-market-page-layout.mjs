...
requireText(marketStyles, '.market-page-surface .market-asset-card__name', '市场商品卡必须提供左上名称槽位。');
requireText(marketStyles, '.market-page-surface .market-asset-card__price', '市场商品卡必须提供右上最近成交价槽位。');
requireText(marketStyles, '.market-page-surface .market-asset-card__artwork', '市场商品卡必须提供居中商品插画槽位。');
requireText(marketStyles, '.market-page-surface .market-asset-card__inventory', '市场商品卡必须提供右下库存槽位。');
requireText(marketStyles, 'grid-template-columns: minmax(0, 1fr) auto', '市场商品卡必须使用名称与价格的两列顶部布局。');
requireText(marketStyles, 'bottom: 6px;\n  left: 8px;', '商品卡当前状态必须移至左下并避让右上价格与右下库存。');
forbidText(productArtworkStyles, 'grid-template-columns: 42px 1fr', '商品插画样式不得重新接管市场商品卡的旧左图右文布局。');
requireText(marketPage, "FactoryIcon, WarehouseIcon", '市场商品库存必须复用统一 WarehouseIcon。');
requireText(marketPage, 'className="market-asset-card__name"', '市场商品卡名称必须使用固定语义槽位。');
requireText(marketPage, 'className="market-asset-card__price"', '市场商品卡价格必须使用固定语义槽位。');
requireText(marketPage, 'className="asset-kind-icon market-asset-card__artwork"', '市场商品插画必须使用固定居中槽位。');
requireText(marketPage, 'className="market-asset-card__inventory"', '市场商品库存必须使用固定右下槽位。');
requireText(marketPage, "const hasLastPrice = typeof lastPrice === 'number';", '市场目录必须显式区分真实最近成交价是否存在。');
requireText(marketPage, "<CurrencyAmount>{hasLastPrice ? formatCurrency(lastPrice) : '—'}</CurrencyAmount>", '市场目录无真实成交时必须显示统一空值。');
...
requireText(designIndex, '商品名称固定左上', '设计索引必须记录市场商品卡名称位置。');
requireText(designIndex, '`CurrencyAmount` 最近真实成交价固定右上', '设计索引必须记录市场商品卡最近成交价位置与组件。');
requireText(designIndex, '`WarehouseIcon` 与可用库存固定右下', '设计索引必须记录市场商品卡库存位置与组件。');
...
console.log('市场页布局、资产卡信息层级、价格档位、数据口径、禁用反馈与共享移动层级浏览器回归基线验证通过。');
