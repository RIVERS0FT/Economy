...
requireText(marketPage, "type MarketCatalogStatus = 'all' | 'traded' | 'buy' | 'sell' | 'owned' | 'own-order';", '市场列表必须提供正式状态筛选。');
requireText(marketPage, "type MarketCatalogSort = 'catalog' | 'name' | 'price' | 'trend' | 'inventory';", '市场列表必须提供正式排序。');
requireText(marketPage, "if (marketViewMode === 'catalog')", '市场必须显式区分列表态和详情态。');
...
requireText(marketPage, "onClick={() => selectMarketAsset(entry.kind, entry.id)}", '市场列表整行必须打开对应资产详情。');
requireText(marketPage, "backAction={{", '市场详情必须复用标题栏返回列表。');
const marketCatalogStart = marketPage.indexOf("if (marketViewMode === 'catalog')");
...
forbidText(marketCatalogSource, '<WidgetHeading', '市场列表不得显示商品／工厂列表标题。');
...
requireText(marketPage, '<FacilityIcon facilityTypeId={entry.id} />', '工厂列表必须使用正式工厂插画。');
requireText(marketPage, "{entry.kind === 'facility' ? <small>{entry.categoryLabel}</small> : null}", '商品名称下必须移除分类说明，工厂仍可显示分类。');
requireText(marketPage, "typeof entry.lastTradePrice === 'number' ? formatCurrency(entry.lastTradePrice) : '—'", '列表无真实成交时必须显示空值。');
requireText(marketStyles, '.market-catalog-row__artwork > .product-artwork {\n  width: 48px;\n  height: 48px;', '市场商品列表插画必须缩小为 48px。');
requireText(marketStyles, '.market-catalog-row__artwork > .facility-icon {\n  width: 58px;\n  height: 58px;', '市场工厂列表插画必须保持 58px。');
...
requireText(facilityArtworkStyles, '.market-catalog-row__artwork', '工厂场景映射必须识别市场列表。');
requireText(facilityArtworkStyles, '.market-detail-hero__artwork', '工厂场景映射必须识别市场详情。');
...
requireText(runtimeSpec, 'market catalog filters products and opens a focused asset detail', 'Playwright 必须覆盖商品筛选与详情入口。');
requireText(runtimeSpec, 'market catalog switches to factory assets without mixing rows', 'Playwright 必须覆盖工厂资产筛选。');
...
requireText(runtimeSpec, "getByRole('heading', { name: '工厂资产列表', exact: true })).toHaveCount(0)", 'Playwright 必须验证市场列表标题已删除。');
...
requireText(pageDesign, '列表态必须支持商品／工厂资产类型、名称搜索、服务器目录分类、市场状态和排序筛选', '页面职责设计必须记录市场筛选。');
requireText(pageDesign, '不显示“商品列表／工厂资产列表”标题', '页面职责设计必须记录列表标题和数量胶囊删除。');
requireText(pageDesign, '商品名称下方不显示分类说明', '页面职责设计必须记录商品名称说明删除。');
requireText(pageDesign, '点击商品或工厂行进入详情', '页面职责设计必须记录列表和详情。');
...
requireText(uiDesign, '市场列表整行是唯一详情入口', 'UI 设计系统必须记录市场列表行。');
requireText(uiDesign, '商品调用 `ProductArtwork` 并固定为 `48px`', 'UI 设计系统必须记录商品列表主视觉。');
requireText(uiDesign, '工厂调用按正式 ID 映射的 `FacilityIcon`、固定为 `58px`', 'UI 设计系统必须记录工厂列表主视觉。');
...
requireText(designIndex, '市场使用“可筛选资产列表 → 资产详情”两级视图', '设计索引必须记录市场两级视图。');
requireText(designIndex, '桌面列表行的 `64px` 身份槽内', '设计索引必须记录市场列表主视觉槽位。');
requireText(designIndex, '返回保留筛选', '设计索引必须记录市场返回语义。');
...
console.log('市场页可筛选资产列表、商品与工厂独立主视觉、详情交易与行情、当前资产订单成交、移动摘要行、连续五档盘口及浏览器回归基线验证通过。');
