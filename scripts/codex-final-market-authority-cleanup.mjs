...
function replaceRequired(path, from, to) {
  ...
  if (!source.includes(from)) throw new Error(`${path}: missing replacement source: ${from.slice(0, 120)}`);
...
function insertBeforeFailure(path, block) {
  let source = read(path);
  if (source.includes(block.trim())) return;
  const marker = /\nif \(failures\.length(?: > 0)?\) \{/;
  if (!marker.test(source)) throw new Error(`${path}: missing final failure marker`);
...
replaceRequired(pageDesign, '概览只展示当前州库存、工厂、运行／异常与挂单摘要；', '概览只展示当前州库存、工厂、运行／异常与经营摘要；');
replaceRequired(
  pageDesign,
...
insertBeforeFailure('scripts/verify-page-content.mjs', `
for (const text of [
  '起始州选择已永久移除',
  '连续 48 州不得按访问资格灰显',
...
replaceRequired('src/components/assets/AssetOverviewPanel.tsx', '商品和工厂按最近一次订单簿真实成交价估值', '商品按当日官方价、工厂按最近产权成交价估值');
replaceRequired(
  'src/pages/LeaderboardPage.tsx',
  '按最近一次订单簿真实成交价计算资产毛值并扣除贷款负债后的实时净资产',
  '商品按当日官方价、工厂按最近产权成交价计算资产毛值并扣除贷款负债后的实时净资产',
);
replaceRequired('src/components/AdminPlayerStatistics.tsx', '商品与工厂只按最近一次订单簿真实成交价估值', '商品按当日官方价、工厂按最近产权成交价估值');
replaceRequired('src/components/AdminPlayerStatistics.tsx', '名玩家持有尚无真实成交估值的商品或工厂。', '名玩家持有尚无真实产权成交估值的工厂。');
...
replaceRequired(adminStats, "['current-trade', '本周有订单簿成交', counts.currentTrade]", "['current-trade', '本周有市场成交', counts.currentTrade]");
replaceRequired(adminStats, "{ id: 'active-no-trade', label: '7 日活跃但本周无订单簿成交', count: activeNoTrade, tone: 'neutral' }", "{ id: 'active-no-trade', label: '7 日活跃但本周无市场成交', count: activeNoTrade, tone: 'neutral' }");

insertBeforeFailure('scripts/verify-assets-page.mjs', `
...
insertBeforeFailure('scripts/verify-leaderboards.mjs', `
...
insertBeforeFailure('scripts/verify-admin-player-statistics.mjs', `
