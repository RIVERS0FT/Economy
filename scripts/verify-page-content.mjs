...
for (const text of [
  ...
  'single-order-book', '最近成交', '最优 5 笔', '我的订单与成交', '本地成交记录', '工厂数量市场',
...
for (const text of ['WarehouseUpgradeCard', 'buildFacility(', 'setProductionPlan(', 'book-columns', 'aggregateOrderBook']) forbidText('src/pages/MarketPage.tsx', text);
...
for (const text of ['title="资金与资产"', '工厂数量资产', '资产配置', '商品库存与估值', '本地资金与资产变动', 'game.facilityGroups']) requireText('src/pages/AssetsPage.tsx', text);
...
for (const text of [
  ...
  '每种 `facilityTypeId` 最多一张卡', '不得显示工厂实例名称、ID、列表或展开实例',
  '资金页不负责仓库管理', '设置页只允许',
...
console.log('页面职责验证通过：工厂集群、紧凑市场和模块唯一归属满足第三版设计。');
