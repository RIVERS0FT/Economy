...
for (const text of ['items={collectibles}','items={giftCodes}','items={ownership}','items={redemptions}','admin-collectibles-virtual-table','admin-gifts-virtual-table','admin-redemptions-virtual-table']) requireText('src/app/AdminApp.tsx', text);
for (const text of ['collectibles.map(','giftCodes.map(','ownership.map(','redemptions.map(']) forbidText('src/app/AdminApp.tsx', text);
for (const text of ['ResizeObserver','overscan','measuredSizesRef','aria-setsize','virtual-list__canvas']) requireText('src/components/ui/VirtualList.tsx', text);
...
for (const text of ["import type { AssetKind, AssetOrder, OrderSide } from '../types'","!['open', 'partial'].includes(order.status) || order.remaining <= 0",'orderKind(order) !== assetKind || orderAssetId(order) !== assetId','Number.isFinite(price) && Number.isInteger(price) && price >= 1','Math.max(bestBid, order.price)','Math.min(bestAsk, order.price)','? bestAsk ?? bestBid ?? 1',': bestBid ?? bestAsk ?? 1']) requireText('src/utils/defaultOrderPrice.ts', text);
...
for (const text of ["import { defaultOrderPrice } from '../utils/defaultOrderPrice'",'setTab: (tab: TabId) => void;','selectOrderSide: (side: OrderSide) => void;',"const [tab, setActiveTab] = useState<TabId>('home');","const [orderSide, setOrderSideState] = useState<OrderSide>('buy');",'const [orderPrice, setOrderPrice] = useState(1);','const loadedGame = game;',"if (nextTab === 'market' && tab !== 'market')",'defaultOrderPrice(loadedGame.orders, marketAssetKind, marketAssetId, orderSide)',"if (changed || tab !== 'market')",'defaultOrderPrice(loadedGame.orders, kind, assetId, orderSide)','function selectOrderSide(side: OrderSide)','defaultOrderPrice(loadedGame.orders, marketAssetKind, marketAssetId, side)']) requireText('src/app/gameViewModel.ts', text);
for (const text of ['const [orderPrice, setOrderPrice] = useState(6);','setOrderPrice(market.lastPrice)','orderSide, setOrderSide,']) forbidText('src/app/gameViewModel.ts', text);
...
for (const text of ['SwitchControl','运行中 <strong>{formatNumber(group.participatingCount)}</strong>','下一周期加入 <strong>{formatNumber(group.pendingJoinCount)}</strong>','冻结中 <strong>{formatNumber(group.listedCount)}</strong>','production-recipe-card','<strong>生产配方</strong>','下一周期切换为：','前往市场交易该工厂 →']) requireText('src/pages/ProductionPage.tsx', text);
for (const text of ['facility-power-button','产成品去向','挂牌数量','单座价格','启动全部未挂牌工厂','停止全部','>保存计划</Button>','下一周期按 ','<span>冻结 <strong>{group.listedCount}</strong></span>']) forbidText('src/pages/ProductionPage.tsx', text);
...
for (const text of ['maker price','反推玩家成交价','逐笔','工厂资产标签使用独立厂房 SVG','玩家界面统一将订单输入字段称为“价格”','默认价格只从客户端当前已经加载的 `game.orders` 本地快照计算','从其他页面重新进入市场页','自动刷新、下单响应、成交、撤单或其他权威状态同步只更新本地订单快照，不得直接覆盖当前价格输入','商品市场允许服务器在某一侧没有有效系统流动性订单时补充一张系统订单','工厂订单只能由玩家提交']) requireText('docs/UNIFIED_ASSET_ORDER_BOOK_DESIGN.md', text);
for (const text of ['玩家可见输入字段、订单标题和未完成订单表头统一使用“价格”','从其他页面重新进入市场','自动刷新和下单后的状态同步不得覆盖当前输入']) requireText('docs/PAGE_CONTENT_AND_NAVIGATION_DESIGN.md', text);
for (const text of ['必须使用共享 `VirtualList` 窗口化组件','DOM 只创建当前滚动视口及少量 `overscan` 范围内的记录','不得用分页、截断、`slice` 或只显示最近记录替代窗口化']) requireText('docs/LOCAL_ACTIVITY_LOG_DESIGN.md', text);
for (const text of ['藏品列表、礼品码列表、归属历史和兑换记录可能持续增长，必须复用共享 `VirtualList`','对管理员藏品、礼品码、归属或兑换记录恢复全量 `.map()` DOM 渲染']) requireText('docs/GIFT_CODE_AND_ADMIN_DESIGN.md', text);
