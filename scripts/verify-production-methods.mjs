...
assert.equal(CURRENT_CLIENT_STATE_VERSION, 38);
assert.equal(MIN_COMPATIBLE_CLIENT_STATE_VERSION, 38);
...
console.log('生产方式验证通过：26 类工厂均使用四种具名制度与语义图标，旧制度只参与等参数存档迁移，共享制度定义、固定精度、研发校验与客户端版本 38 均已锁定。');
