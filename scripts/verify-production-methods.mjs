...
assert.equal(CURRENT_CLIENT_STATE_VERSION, 45);
assert.equal(MIN_COMPATIBLE_CLIENT_STATE_VERSION, 45);
...
console.log('生产方式验证通过：26 类工厂均使用四种具名制度与语义图标，旧制度只参与等参数存档迁移，共享制度定义、固定精度、研发校验、独立移动视口门禁与客户端版本 45 均已锁定。');
